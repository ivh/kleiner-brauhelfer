...
MainWindowImpl::MainWindowImpl(QWidget *parent, Qt::WindowFlags f)
  ...
  connect(graphicsView_bewStar, SIGNAL(sig_AnzahlStarChanged(int)), this,
          SLOT(slot_GraphicsView_AnzahlSterneChanged(int)));

  // verbinde Button Neuer Eintrag Geräte mit Funktion;
  connect(pushButton_GeraeteNeu, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_GeraeteNeu()));
  // verbinde Button Eintrag Löschen Geräte mit Funktion;
  connect(pushButton_GeraeteDel, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_GeraeteDel()));

  // verbinde Button Sud wurde Gebraut mit Funktion;
  connect(pushButton_RohstoffeAbziehen, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_gebraut()));

  // verbinde Button Sud wurde Abgefüllt mit Funktion;
  connect(pushButton_SudAbgefuellt, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_SudAbgefuellt()));

  // verbinde Button Braudatum Heute mit Funktion
  connect(pushButton_BraudatumHeute, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_BraudatumHeute()));

  // verbinde Button Anstelldatum Heute mit Funktion
  connect(pushButton_AnstelldatumHeute, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_AnstelldatumHeute()));

  // verbinde Button Braudatum Heute mit Funktion
  connect(pushButton_AbfuelldatumHeute, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_AbfuelldatumHeute()));

  // verbinde Button Eingabehilfe Stammwürze Kochende
  connect(pushButton_EingabeHSWKochende, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_EingabeHSWKochende()));

  // verbinde Button Eingabehilfe Stammwürze Anstellen
  connect(pushButton_EingabeHSWAnstellen, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_EingabeHSWAnstellen()));

  // verbinde Button Eingabehilfe Stammwürze Schnellgärprobe
  connect(pushButton_EingabeHSWSchnellgaerprobe, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_EingabeHSWSchnellgaerprobe()));

  // verbinde Button Eingabehilfe Stammwürze Jungbier
  connect(pushButton_EingabeHSWJungbier, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_EingabeHSWJungbier()));

  // verbinde Button Neuen Sud Anlegen mit Funkiton
  connect(pushButton_SudNeu, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_SudNeu()));
  // verbinde Button Neuen Sud löschen mit Funkiton
  connect(pushButton_SudDel, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_SudDel()));
  // verbinde Button Neuen Sud Laden mit Funkiton
  connect(pushButton_SudLaden, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_SudLaden()));
  // verbinde Button Sud Kopieren mit Funkiton
  connect(pushButton_SudKopie, SIGNAL(clicked()), this,
          SLOT(slot_pushButton_SudKopie()));

  // verbinde Filter Radiobutton mit Funktion
  connect(radioButton_FilterAlle, SIGNAL(clicked(bool)), this,
          SLOT(slot_FilterClicked(bool)));
  connect(radioButton_FilterNichtGebraut, SIGNAL(clicked(bool)), this,
          SLOT(slot_FilterClicked(bool)));
  connect(radioButton_FilterGebrautNichtAbgefuellt, SIGNAL(clicked(bool)), this,
          SLOT(slot_FilterClicked(bool)));
  connect(radioButton_Abgefuellt, SIGNAL(clicked(bool)), this,
          SLOT(slot_FilterClicked(bool)));
  connect(radioButton_nichtVerbraucht, SIGNAL(clicked(bool)), this,
          SLOT(slot_FilterClicked(bool)));
  connect(radioButton_Merkliste, SIGNAL(clicked(bool)), this,
          SLOT(slot_FilterClicked(bool)));

  // verbinde changed signale mit gemeinsammer funktion zur Neuberechnung der
  // Daten
  connect(spinBox_AngenommeneAusbeute, SIGNAL(valueChanged(int)), this,
          SLOT(slot_spinBoxValueChanged(int)));
  connect(spinBox_SW, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_Gesammtkochdauer, SIGNAL(valueChanged(int)), this,
          SLOT(slot_spinBoxValueChanged(int)));
  connect(spinBox_IBU, SIGNAL(valueChanged(int)), this,
          SLOT(slot_spinBoxValueChanged(int)));
  connect(spinBox_EinmaischenTemp, SIGNAL(valueChanged(int)), this,
          SLOT(slot_spinBoxValueChanged(int)));
  connect(spinBox_Reifezeit, SIGNAL(valueChanged(int)), this,
          SLOT(slot_spinBoxValueChanged(int)));
  connect(spinBox_NachisomerisierungsZeit, SIGNAL(valueChanged(int)), this,
          SLOT(slot_spinBoxValueChanged(int)));
  connect(spinBox_AnzahlHefeEinheiten, SIGNAL(valueChanged(int)), this,
          SLOT(slot_spinBoxValueChanged(int)));

  connect(checkBox_SchnellgaerprobeAktiv, SIGNAL(stateChanged(int)), this,
          SLOT(slot_spinBoxValueChanged(int)));
  connect(checkBox_Spunden, SIGNAL(stateChanged(int)), this,
          SLOT(slot_spinBoxValueChanged(int)));

  // Setzt den Maxwert Zeit für die Hopfengaben
  connect(spinBox_Gesammtkochdauer, SIGNAL(valueChanged(int)), this,
          SLOT(slot_spinBoxGesammtkochdauerChanged(int)));

  connect(doubleSpinBox_Verdampfung, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(dspinBox_KostenAusruestung, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(dSpinBox_KorrekturNachguss, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_Menge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_WuerzemengeKochende, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_SWKochende, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_Speisemenge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_WuerzemengeAnstellen, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_SWAnstellen, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_SWSchnellgaerprobe, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_SWJungbier, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_TemperaturJungbier, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(doubleSpinBox_CO2, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(doubleSpinBox_FaktorHG, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_Nebenkosten, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_WuerzemengeVorHopfenseihen, SIGNAL(valueChanged(double)),
          this, SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_SWVorHopfenseihen, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  ...
  connect(spinBox_MaischebottichDurchmesser, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_MaischebottichMaxFuellhoehe, SIGNAL(valueChanged(double)),
          this, SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_SudpfanneHoehe, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_SudpfanneDurchmesser, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(spinBox_SudpfanneMaxFuellhoehe, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));

  connect(SpinBox_waSollRestalkalitaet_dh, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(SpinBox_wwCalcium_mg, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(SpinBox_wwMagnesium_mg, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));
  connect(SpinBox_wwSaeurekapazitaet_mmol, SIGNAL(valueChanged(double)), this,
          SLOT(slot_spinBoxValueChanged(double)));

  connect(comboBox_AuswahlHefe, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slot_ComboBoxIndexChanged(int)));
  connect(comboBox_AuswahlBrauanlage, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slot_ComboBoxIndexChanged(int)));
  ...
  connect(tableWidget_Hopfen, SIGNAL(cellChanged(int, int)), this,
          SLOT(slot_TableWidget_cellChanged(int, int)));
  connect(tableWidget_Hefe, SIGNAL(cellChanged(int, int)), this,
          SLOT(slot_TableWidget_cellChanged(int, int)));
  connect(tableWidget_WeitereZutaten, SIGNAL(cellChanged(int, int)), this,
          SLOT(slot_TableWidget_cellChanged(int, int)));

  // Tab wird gewechselt
  connect(tabWidged, SIGNAL(currentChanged(int)), this,
          SLOT(slot_tabWidgetChanged(int)));

  // SpinBox Stammwürze nach Kochende mit SpinBox Stammwüzre vor dem
  // Hopfenseihen verbinden
  connect(spinBox_SWKochende, SIGNAL(valueChanged(double)),
          spinBox_SWVorHopfenseihen, SLOT(setValue(double)));
...
void MainWindowImpl::initUi() {
  ...
  loadSprache(
      settings.value("sprachauswahl", QLocale::system().name()).toString());
  settings.endGroup();
  ...
  tableWidget_Brauuebersicht->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Interactive);
  ...
  // tableWidget_Malz
  tableWidget_Malz->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Interactive);
  tableWidget_Malz->horizontalHeader()->resizeSection(TableMalzColName, 200);
  tableWidget_Malz->horizontalHeader()->resizeSection(TableMalzColBemerkung,
                                                      200);
  tableWidget_Malz->horizontalHeader()->resizeSection(TableMalzColAnwendung,
                                                      200);
  ...
  // tableWidget_Hopfen
  tableWidget_Hopfen->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Interactive);
  tableWidget_Hopfen->horizontalHeader()->resizeSection(TableHopfenColName,
                                                        200);
  tableWidget_Hopfen->horizontalHeader()->resizeSection(TableHopfenColBemerkung,
                                                        200);
  tableWidget_Hopfen->horizontalHeader()->resizeSection(
      TableHopfenColEigenschaften, 200);
  tableWidget_Hopfen->horizontalHeader()->resizeSection(TableHopfenColLink,
                                                        100);
  ...
  // tableWidget_Hefe
  tableWidget_Hefe->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Interactive);
  tableWidget_Hefe->horizontalHeader()->resizeSection(TableHefeColName, 200);
  tableWidget_Hefe->horizontalHeader()->resizeSection(TableHefeColBemerkung,
                                                      200);
  tableWidget_Hefe->horizontalHeader()->resizeSection(TableHefeColEigenschaften,
                                                      200);
  ...
  tableWidget_WeitereZutaten->horizontalHeader()->setSectionResizeMode(
      QHeaderView::Interactive);
  tableWidget_WeitereZutaten->horizontalHeader()->resizeSection(
      TableWZutatColName, 200);
  tableWidget_WeitereZutaten->horizontalHeader()->resizeSection(
      TableWZutatColBemerkung, 200);
  tableWidget_WeitereZutaten->horizontalHeader()->resizeSection(
      TableWZutatColLink, 100);
...
void MainWindowImpl::retranslate() {
  ...
  tabWidged->setTabText(tabWidged->indexOf(tab_Spickzettel),
                        BierWurdeGebraut ? trUtf8("Zusammenfassung")
                                         : trUtf8("Spickzettel"));
}
...
void MainWindowImpl::closeEvent(QCloseEvent *evt) {
  ...
  if (Aenderung)
    ...
    bool abfrage = settings.value("checkBox_keineBeendenAbfrage").toBool();
    settings.endGroup();
    if (!abfrage) {
      close =
...
int MainWindowImpl::getBrauanlagenIDRezept() {
  ...
  for (int i = 0; i < listWidget_Brauanlagen->count(); i++) {
    Brauanlage *item =
        dynamic_cast<Brauanlage *>(listWidget_Brauanlagen->item(i));
...
int MainWindowImpl::getBrauanlagenIDAusruestung() {
  int id = 0;
  Brauanlage *item =
      dynamic_cast<Brauanlage *>(listWidget_Brauanlagen->currentItem());
  id = item->getID();
  return id;
}
...
void MainWindowImpl::setAngenommeneSudhausausbeute(int value) {
  Brauanlage *item = dynamic_cast<Brauanlage *>(
      listWidget_Brauanlagen->selectedItems().first());
  if (item != 0) {
...
double MainWindowImpl::getMaischenMaxNutzvolumen() {
  ...
  return double(qRound((Grundflaeche * getMaischebottichMaxFuellhoehe() / 100) *
                       100)) /
         100;
}
...
double MainWindowImpl::getSudpfanneMaxNutzvolumen() {
  ...
  return double(
             qRound((Grundflaeche * getSudpfanneMaxFuellhoehe() / 100) * 100)) /
         100;
}

void MainWindowImpl::switchTranslator(QTranslator &translator,
...
void MainWindowImpl::loadSprache(const QString &rLanguage) {
  if (m_currLang != rLanguage) {
    ...
    switchTranslator(m_translator,
                     QString(langPath + "/kb_%1.qm").arg(m_currLang));
    switchTranslator(m_translatorQt,
                     QString(langPath + "/qt_%1.qm").arg(m_currLang));
  }
}
...
void MainWindowImpl::checkMsg() {
  ...
  if (!b) {
    ...
    connect(&msgdlg, SIGNAL(sig_checkFertig(int)), this,
            SLOT(on_MsgCheckFertig(int)));
  }
}
...
void MainWindowImpl::SchreibeAusruestungDB() {
  if (AenderungAusruestung) {
    ...
    for (int i = 0; i < listWidget_Brauanlagen->count(); i++) {
      Brauanlage *item =
          dynamic_cast<Brauanlage *>(listWidget_Brauanlagen->item(i));
      sql = "INSERT INTO 'Ausruestung' ('Name', 'AnlagenID', "
            "'Maischebottich_Hoehe','Maischebottich_Durchmesser','"
            "Maischebottich_MaxFuellhoehe','Sudpfanne_Hoehe','Sudpfanne_"
            "Durchmesser','Sudpfanne_MaxFuellhoehe','KorrekturWasser','"
            "KorrekturFarbe','Verdampfungsziffer', 'Kosten', "
            "'Sudhausausbeute') ";
      sql += "VALUES (";
...
void MainWindowImpl::MalzNeueZeile(const QString &name, double ebc,
                                   double schuettung, double menge,
                                   double preis, const QString &bemerkung,
                                   const QString &anwendung,
                                   const QDate &einlagerung,
                                   const QDate &haltbar, const QString &link) {
  ...
  tableWidget_Malz->setItem(i, TableMalzColFarbe, new MyTableWidgetItemNumeric(
                                                      spinBoxFarbe->value()));
  spinBoxFarbe->setProperty("TableWidgetItem",
                            QVariant::fromValue(static_cast<void *>(
                                tableWidget_Malz->item(i, TableMalzColFarbe))));
  connect(spinBoxFarbe, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));
  connect(spinBoxFarbe, SIGNAL(valueChanged(double)), this,
          SLOT(slot_RohstoffFarbeValueChanged(double)));
  ...
  tableWidget_Malz->setCellWidget(i, TableMalzColMaxSchuettung,
                                  spinBoxMaxSchuettung);
  tableWidget_Malz->setItem(
      i, TableMalzColMaxSchuettung,
      new MyTableWidgetItemNumeric(spinBoxMaxSchuettung->value()));
  spinBoxMaxSchuettung->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Malz->item(i, TableMalzColMaxSchuettung))));
  connect(spinBoxMaxSchuettung, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));
  ...
  tableWidget_Malz->setItem(i, TableMalzColMenge, new MyTableWidgetItemNumeric(
                                                      spinBoxMenge->value()));
  spinBoxMenge->setProperty("TableWidgetItem",
                            QVariant::fromValue(static_cast<void *>(
                                tableWidget_Malz->item(i, TableMalzColMenge))));
  connect(spinBoxMenge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));
  connect(spinBoxMenge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_RohstoffMengeValueChanged(double)));
  ...
  tableWidget_Malz->setItem(i, TableMalzColPreis, new MyTableWidgetItemNumeric(
                                                      spinBoxPreis->value()));
  spinBoxPreis->setProperty("TableWidgetItem",
                            QVariant::fromValue(static_cast<void *>(
                                tableWidget_Malz->item(i, TableMalzColPreis))));
  connect(spinBoxPreis, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));

  // Bemerkung
  tableWidget_Malz->setItem(i, TableMalzColBemerkung,
                            new QTableWidgetItem(bemerkung));

  // Anwendung
  tableWidget_Malz->setItem(i, TableMalzColAnwendung,
                            new QTableWidgetItem(anwendung));
  ...
  connect(deEinlagerung, SIGNAL(dateChanged(QDate)), this,
          SLOT(slot_dateChanged(QDate)));
  tableWidget_Malz->setCellWidget(i, TableMalzColEinlagerung, deEinlagerung);
  tableWidget_Malz->setItem(
      i, TableMalzColEinlagerung,
      new MyTableWidgetItemNumeric(deEinlagerung->date().toJulianDay()));
  deEinlagerung->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Malz->item(i, TableMalzColEinlagerung))));
  connect(deEinlagerung, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_tableSyncValueChanged(const QDate &)));
  ...
  connect(deMhd, SIGNAL(dateChanged(QDate)), this,
          SLOT(slot_dateChanged(QDate)));
  tableWidget_Malz->setCellWidget(i, TableMalzColMindesthalbat, deMhd);
  tableWidget_Malz->setItem(
      i, TableMalzColMindesthalbat,
      new MyTableWidgetItemNumeric(deMhd->date().toJulianDay()));
  deMhd->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Malz->item(i, TableMalzColMindesthalbat))));
  connect(deMhd, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_tableSyncValueChanged(const QDate &)));
  connect(deMhd, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_RohstoffHaltbarValueChanged(const QDate &)));
...
void MainWindowImpl::on_pushButton_MalzNeuVorlage_clicked() {
  ...
  if (grvDia->exec() == QDialog::Accepted) {
    ...
    MalzNeueZeile(grvDia->m_Beschreibung, grvDia->m_Farbe, grvDia->m_MaxProzent,
                  0.0, 0.0, "", grvDia->m_Eigenschaften, QDate::currentDate(),
...
void MainWindowImpl::HopfenNeueZeile(
    const QString &name, double alpha, double menge, double preis, bool pellets,
  ...
  // Beschreibung
  tableWidget_Hopfen->setItem(i, TableHopfenColName,
  ...
  tableWidget_Hopfen->setItem(
      i, TableHopfenColAlpha,
      new MyTableWidgetItemNumeric(spinBoxAlpha->value()));
  spinBoxAlpha->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Hopfen->item(i, TableHopfenColAlpha))));
  connect(spinBoxAlpha, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));
  ...
  tableWidget_Hopfen->setItem(
      i, TableHopfenColMenge,
      new MyTableWidgetItemNumeric(spinBoxMenge->value()));
  spinBoxMenge->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Hopfen->item(i, TableHopfenColMenge))));
  connect(spinBoxMenge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));
  connect(spinBoxMenge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_RohstoffMengeValueChanged(double)));
  ...
  tableWidget_Hopfen->setItem(
      i, TableHopfenColPreis,
      new MyTableWidgetItemNumeric(spinBoxPreis->value()));
  spinBoxPreis->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Hopfen->item(i, TableHopfenColPreis))));
  connect(spinBoxPreis, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));

  // Pellets
  QTableWidgetItem *newItem4 =
      new QTableWidgetItem(trUtf8("Pellets") + (pellets ? " " : ""));
  ...
  // Bemerkung
  tableWidget_Hopfen->setItem(i, TableHopfenColBemerkung,
                              new QTableWidgetItem(bemerkung));
  ...
  tableWidget_Hopfen->setItem(i, TableHopfenColTyp,
                              new QTableWidgetItem(comboBoxTyp->currentText()));
  comboBoxTyp->setProperty(
      "TableWidgetItem", QVariant::fromValue(static_cast<void *>(
                             tableWidget_Hopfen->item(i, TableHopfenColTyp))));
  connect(comboBoxTyp, SIGNAL(currentIndexChanged(const QString &)), this,
          SLOT(slot_tableSyncValueChanged(const QString &)));

  // Eigenschaften
  tableWidget_Hopfen->setItem(i, TableHopfenColEigenschaften,
                              new QTableWidgetItem(eigenschaft));
  ...
  connect(deEinlagerung, SIGNAL(dateChanged(QDate)), this,
          SLOT(slot_dateChanged(QDate)));
  tableWidget_Hopfen->setCellWidget(i, TableHopfenColEinlagerung,
                                    deEinlagerung);
  tableWidget_Hopfen->setItem(
      i, TableHopfenColEinlagerung,
      new MyTableWidgetItemNumeric(deEinlagerung->date().toJulianDay()));
  deEinlagerung->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Hopfen->item(i, TableHopfenColEinlagerung))));
  connect(deEinlagerung, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_tableSyncValueChanged(const QDate &)));
  ...
  tableWidget_Hopfen->setItem(
      i, TableHopfenColMindesthalbat,
      new MyTableWidgetItemNumeric(deMhd->date().toJulianDay()));
  deMhd->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Hopfen->item(i, TableHopfenColMindesthalbat))));
  connect(deMhd, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_tableSyncValueChanged(const QDate &)));
  connect(deMhd, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_RohstoffHaltbarValueChanged(const QDate &)));
...
void MainWindowImpl::on_pushButton_HopfenNeuVorlage_clicked() {
  ...
  if (grvDia->exec() == QDialog::Accepted) {
    ...
    HopfenNeueZeile(grvDia->m_Beschreibung, grvDia->m_Alpha, 0.0, 0.0, true, "",
                    grvDia->m_Typ, grvDia->m_Eigenschaften,
...
void MainWindowImpl::HefeNeueZeile(
    const QString &name, double menge, double wuerzemenge, double preis,
    const QString &bemerkung, const QString &verpackung, int typ1, int typ2,
    const QString &temperatur, const QString &eigenschaft, int sedimentation,
    const QString &evg, const QDate &einlagerung, const QDate &haltbar,
    const QString &link) {
  ...
  tableWidget_Hefe->setItem(i, TableHefeColMenge, new MyTableWidgetItemNumeric(
                                                      spinBoxMenge->value()));
  spinBoxMenge->setProperty("TableWidgetItem",
                            QVariant::fromValue(static_cast<void *>(
                                tableWidget_Hefe->item(i, TableHefeColMenge))));
  connect(spinBoxMenge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));
  connect(spinBoxMenge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_RohstoffMengeValueChanged(double)));
  ...
  tableWidget_Hefe->setCellWidget(i, TableHefeColWuerzemenge,
                                  spinBoxWuerzemenge);
  tableWidget_Hefe->setItem(
      i, TableHefeColWuerzemenge,
      new MyTableWidgetItemNumeric(spinBoxWuerzemenge->value()));
  spinBoxWuerzemenge->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Hefe->item(i, TableHefeColWuerzemenge))));
  connect(spinBoxWuerzemenge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));
  connect(spinBoxWuerzemenge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_RohstoffNonZeroValueChanged(double)));
  ...
  tableWidget_Hefe->setItem(i, TableHefeColPreis, new MyTableWidgetItemNumeric(
                                                      spinBoxPreis->value()));
  spinBoxPreis->setProperty("TableWidgetItem",
                            QVariant::fromValue(static_cast<void *>(
                                tableWidget_Hefe->item(i, TableHefeColPreis))));
  connect(spinBoxPreis, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));

  // Bemerkung
  tableWidget_Hefe->setItem(i, TableHefeColBemerkung,
                            new QTableWidgetItem(bemerkung));

  // Verpackungsmenge
  tableWidget_Hefe->setItem(i, TableHefeColVerpackungsmenge,
                            new QTableWidgetItem(verpackung));
  ...
  tableWidget_Hefe->setItem(
      i, TableHefeColTypUgOg,
      new QTableWidgetItem(comboBoxTypOGUG->currentText()));
  comboBoxTypOGUG->setProperty(
      "TableWidgetItem", QVariant::fromValue(static_cast<void *>(
                             tableWidget_Hefe->item(i, TableHefeColTypUgOg))));
  connect(comboBoxTypOGUG, SIGNAL(currentIndexChanged(const QString &)), this,
          SLOT(slot_tableSyncValueChanged(const QString &)));
  ...
  tableWidget_Hefe->setItem(
      i, TableHefeColTypTrFlg,
      new QTableWidgetItem(comboBoxTypTrFl->currentText()));
  comboBoxTypTrFl->setProperty(
      "TableWidgetItem", QVariant::fromValue(static_cast<void *>(
                             tableWidget_Hefe->item(i, TableHefeColTypTrFlg))));
  connect(comboBoxTypTrFl, SIGNAL(currentIndexChanged(const QString &)), this,
          SLOT(slot_tableSyncValueChanged(const QString &)));

  // Temperaturbereich
  tableWidget_Hefe->setItem(i, TableHefeColTemperatur,
                            new QTableWidgetItem(temperatur));

  // Eigenschaften
  tableWidget_Hefe->setItem(i, TableHefeColEigenschaften,
                            new QTableWidgetItem(eigenschaft));
  ...
  tableWidget_Hefe->setItem(i, TableHefeColSedimentation,
                            new QTableWidgetItem(comboBoxSED->currentText()));
  comboBoxSED->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Hefe->item(i, TableHefeColSedimentation))));
  connect(comboBoxSED, SIGNAL(currentIndexChanged(const QString &)), this,
          SLOT(slot_tableSyncValueChanged(const QString &)));
  ...
  tableWidget_Hefe->setItem(
      i, TableHefeColEinlagerung,
      new MyTableWidgetItemNumeric(deEinlagerung->date().toJulianDay()));
  deEinlagerung->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Hefe->item(i, TableHefeColEinlagerung))));
  connect(deEinlagerung, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_tableSyncValueChanged(const QDate &)));
  ...
  tableWidget_Hefe->setItem(
      i, TableHefeColMindesthalbat,
      new MyTableWidgetItemNumeric(deMhd->date().toJulianDay()));
  deMhd->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_Hefe->item(i, TableHefeColMindesthalbat))));
  connect(deMhd, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_tableSyncValueChanged(const QDate &)));
  connect(deMhd, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_RohstoffHaltbarValueChanged(const QDate &)));
...
void MainWindowImpl::on_pushButton_HefeNeuVorlage_clicked() {
  ...
  if (grvDia->exec() == QDialog::Accepted) {
    ...
    HefeNeueZeile(grvDia->m_Beschreibung, 0.0, grvDia->m_Wuerzemenge, 0.0, "",
                  grvDia->m_Verpackungsmenge, grvDia->m_TypOGUG,
                  grvDia->m_TypFlTr, grvDia->m_Temperatur,
                  grvDia->m_Eigenschaften, grvDia->m_SED, grvDia->m_EVG,
                  QDate::currentDate(), QDate::currentDate().addMonths(1), "");
    setAenderung(true);
    AenderungRohstofftabelle = true;
...
void MainWindowImpl::WeitereZutatNeueZeile(
    const QString &name, double menge, int einheit, int typ, double ausbeute,
    double ebc, double preis, const QString &bemerkung,
    const QDate &einlagerung, const QDate &haltbar, const QString &link) {
  int i = tableWidget_WeitereZutaten->rowCount();
  tableWidget_WeitereZutaten->setRowCount(i + 1);

  // Beschreibung
  tableWidget_WeitereZutaten->setItem(i, TableWZutatColName,
                                      new QTableWidgetItem(name));
  ...
  tableWidget_WeitereZutaten->setCellWidget(i, TableWZutatColMenge,
                                            spinBoxMenge);
  tableWidget_WeitereZutaten->setItem(
      i, TableWZutatColMenge,
      new MyTableWidgetItemNumeric(spinBoxMenge->value()));
  spinBoxMenge->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_WeitereZutaten->item(i, TableWZutatColMenge))));
  connect(spinBoxMenge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));
  connect(spinBoxMenge, SIGNAL(valueChanged(double)), this,
          SLOT(slot_RohstoffMengeValueChanged(double)));
  ...
  tableWidget_WeitereZutaten->setCellWidget(i, TableWZutatColEinheit,
                                            comboBoxEinheiten);
  tableWidget_WeitereZutaten->setItem(
      i, TableWZutatColEinheit,
      new QTableWidgetItem(comboBoxEinheiten->currentText()));
  comboBoxEinheiten->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_WeitereZutaten->item(i, TableWZutatColEinheit))));
  connect(comboBoxEinheiten, SIGNAL(currentIndexChanged(const QString &)), this,
          SLOT(slot_tableSyncValueChanged(const QString &)));
  ...
  tableWidget_WeitereZutaten->setItem(
      i, TableWZutatColTyp, new QTableWidgetItem(comboBoxTyp->currentText()));
  comboBoxTyp->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_WeitereZutaten->item(i, TableWZutatColTyp))));
  connect(comboBoxTyp, SIGNAL(currentIndexChanged(const QString &)), this,
          SLOT(slot_tableSyncValueChanged(const QString &)));
  ...
  tableWidget_WeitereZutaten->setCellWidget(i, TableWZutatColAusbeute,
                                            spinBoxAusbeute);
  tableWidget_WeitereZutaten->setItem(
      i, TableWZutatColAusbeute,
      new MyTableWidgetItemNumeric(spinBoxAusbeute->value()));
  spinBoxAusbeute->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_WeitereZutaten->item(i, TableWZutatColAusbeute))));
  connect(spinBoxAusbeute, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));
  ...
  spinBoxFarbe->setDecimals(1);
  spinBoxFarbe->setValue(ebc);
  tableWidget_WeitereZutaten->setCellWidget(i, TableWZutatColFarbe,
                                            spinBoxFarbe);
  tableWidget_WeitereZutaten->setItem(
      i, TableWZutatColFarbe,
      new MyTableWidgetItemNumeric(spinBoxFarbe->value()));
  spinBoxFarbe->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_WeitereZutaten->item(i, TableWZutatColFarbe))));
  connect(spinBoxFarbe, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));
  connect(spinBoxFarbe, SIGNAL(valueChanged(double)), this,
          SLOT(slot_RohstoffFarbeValueChanged(double)));
  ...
  tableWidget_WeitereZutaten->setCellWidget(i, TableWZutatColPreis,
                                            spinBoxPreis);
  tableWidget_WeitereZutaten->setItem(
      i, TableWZutatColPreis,
      new MyTableWidgetItemNumeric(spinBoxPreis->value()));
  spinBoxPreis->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_WeitereZutaten->item(i, TableWZutatColPreis))));
  connect(spinBoxPreis, SIGNAL(valueChanged(double)), this,
          SLOT(slot_tableSyncValueChanged(double)));

  // Bemerkung
  tableWidget_WeitereZutaten->setItem(i, TableWZutatColBemerkung,
                                      new QTableWidgetItem(bemerkung));
  ...
  connect(deEinlagerung, SIGNAL(dateChanged(QDate)), this,
          SLOT(slot_dateChanged(QDate)));
  tableWidget_WeitereZutaten->setCellWidget(i, TableWZutatColEinlagerung,
                                            deEinlagerung);
  tableWidget_WeitereZutaten->setItem(
      i, TableWZutatColEinlagerung,
      new MyTableWidgetItemNumeric(deEinlagerung->date().toJulianDay()));
  deEinlagerung->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_WeitereZutaten->item(i, TableWZutatColEinlagerung))));
  connect(deEinlagerung, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_tableSyncValueChanged(const QDate &)));
  ...
  connect(deMhd, SIGNAL(dateChanged(QDate)), this,
          SLOT(slot_dateChanged(QDate)));
  tableWidget_WeitereZutaten->setCellWidget(i, TableWZutatColMindesthalbat,
                                            deMhd);
  tableWidget_WeitereZutaten->setItem(
      i, TableWZutatColMindesthalbat,
      new MyTableWidgetItemNumeric(deMhd->date().toJulianDay()));
  deMhd->setProperty(
      "TableWidgetItem",
      QVariant::fromValue(static_cast<void *>(
          tableWidget_WeitereZutaten->item(i, TableWZutatColMindesthalbat))));
  connect(deMhd, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_tableSyncValueChanged(const QDate &)));
  connect(deMhd, SIGNAL(dateChanged(const QDate &)), this,
          SLOT(slot_RohstoffHaltbarValueChanged(const QDate &)));
...
void MainWindowImpl::SchreibeRohstoffeDB() {
  ...
  for (int i = 0; i < tableWidget_Malz->rowCount(); i++) {
    QDoubleSpinBox *spinBoxFarbe =
        (QDoubleSpinBox *)tableWidget_Malz->cellWidget(i, TableMalzColFarbe);
    QDoubleSpinBox *spinBoxMaxSchuettung =
        (QDoubleSpinBox *)tableWidget_Malz->cellWidget(
            i, TableMalzColMaxSchuettung);
    QDoubleSpinBox *spinBoxMenge =
        (QDoubleSpinBox *)tableWidget_Malz->cellWidget(i, TableMalzColMenge);
    QDoubleSpinBox *spinBoxPreis =
        (QDoubleSpinBox *)tableWidget_Malz->cellWidget(i, TableMalzColPreis);
    QDateEdit *deEinlagerung =
        (QDateEdit *)tableWidget_Malz->cellWidget(i, TableMalzColEinlagerung);
    QDateEdit *deMhd =
        (QDateEdit *)tableWidget_Malz->cellWidget(i, TableMalzColMindesthalbat);
    sql =
        "INSERT INTO Malz(Beschreibung, Farbe, MaxProzent, Menge, Preis, "
        "Bemerkung, Anwendung, Eingelagert, Mindesthaltbar, Link) VALUES(\'" +
        ...
        tableWidget_Malz->item(i, TableMalzColBemerkung)
            ->text()
            .replace("'", "''") +
        "\',\'" +
        tableWidget_Malz->item(i, TableMalzColAnwendung)
            ->text()
            .replace("'", "''") +
  ...
  for (int i = 0; i < tableWidget_Hopfen->rowCount(); i++) {
    ...
    QDoubleSpinBox *spinBoxAlpha =
        (QDoubleSpinBox *)tableWidget_Hopfen->cellWidget(i,
                                                         TableHopfenColAlpha);
    QDoubleSpinBox *spinBoxMenge =
        (QDoubleSpinBox *)tableWidget_Hopfen->cellWidget(i,
                                                         TableHopfenColMenge);
    QDoubleSpinBox *spinBoxPreis =
        (QDoubleSpinBox *)tableWidget_Hopfen->cellWidget(i,
                                                         TableHopfenColPreis);
    QComboBox *comboTyp =
        (QComboBox *)tableWidget_Hopfen->cellWidget(i, TableHopfenColTyp);
    QDateEdit *deEinlagerung = (QDateEdit *)tableWidget_Hopfen->cellWidget(
        i, TableHopfenColEinlagerung);
    QDateEdit *deMhd = (QDateEdit *)tableWidget_Hopfen->cellWidget(
        i, TableHopfenColMindesthalbat);
    sql = "INSERT INTO Hopfen(Beschreibung, Alpha, Menge, Preis, Pellets, "
          "Bemerkung, Eigenschaften, Typ, Eingelagert, Mindesthaltbar, Link) "
          "VALUES(\'" +
          tableWidget_Hopfen->item(i, TableHopfenColName)
              ->text()
              .replace("'", "''") +
          "\'," + QString::number(spinBoxAlpha->value()) + "," +
          QString::number(spinBoxMenge->value()) + "," +
          QString::number(spinBoxPreis->value()) + "," + QString::number(b) +
          ",\'" +
          tableWidget_Hopfen->item(i, TableHopfenColBemerkung)
              ->text()
              .replace("'", "''") +
          "\',\'" +
          tableWidget_Hopfen->item(i, TableHopfenColEigenschaften)
              ->text()
              .replace("'", "''") +
          "\'," + QString::number(comboTyp->currentIndex()) + ",'" +
          deEinlagerung->date().toString(Qt::ISODate) + "','" +
          deMhd->date().toString(Qt::ISODate) + "','" +
          tableWidget_Hopfen->item(i, TableHopfenColLink)
              ->text()
              .replace("'", "''") +
          "')";
    if (!query.exec(sql)) {
  ...
  for (int i = 0; i < tableWidget_Hefe->rowCount(); i++) {
    QDoubleSpinBox *spinBoxMenge =
        (QDoubleSpinBox *)tableWidget_Hefe->cellWidget(i, TableHefeColMenge);
    QDoubleSpinBox *spinBoxWuerzemenge =
        (QDoubleSpinBox *)tableWidget_Hefe->cellWidget(i,
                                                       TableHefeColWuerzemenge);
    QDoubleSpinBox *spinBoxPreis =
        (QDoubleSpinBox *)tableWidget_Hefe->cellWidget(i, TableHefeColPreis);
    QComboBox *comboTypOGUG =
        (QComboBox *)tableWidget_Hefe->cellWidget(i, TableHefeColTypUgOg);
    QComboBox *comboTypTrFl =
        (QComboBox *)tableWidget_Hefe->cellWidget(i, TableHefeColTypTrFlg);
    QComboBox *comboSED =
        (QComboBox *)tableWidget_Hefe->cellWidget(i, TableHefeColSedimentation);
    QDateEdit *deEinlagerung =
        (QDateEdit *)tableWidget_Hefe->cellWidget(i, TableHefeColEinlagerung);
    QDateEdit *deMhd =
        (QDateEdit *)tableWidget_Hefe->cellWidget(i, TableHefeColMindesthalbat);
    sql =
        "INSERT INTO Hefe(Beschreibung, Menge, Wuerzemenge, Preis, Bemerkung, "
        "Verpackungsmenge, TypOGUG, TypTrFl, Temperatur, Eigenschaften, SED, "
        "EVG, Eingelagert, Mindesthaltbar, Link) VALUES(\'" +
        tableWidget_Hefe->item(i, TableHefeColName)->text().replace("'", "''") +
        "\'," + QString::number(spinBoxMenge->value()) + "," +
        QString::number(spinBoxWuerzemenge->value()) + "," +
        QString::number(spinBoxPreis->value()) + ",\'" +
        tableWidget_Hefe->item(i, TableHefeColBemerkung)
            ->text()
            .replace("'", "''") +
        "\',\'" +
        tableWidget_Hefe->item(i, TableHefeColVerpackungsmenge)
            ->text()
            .replace("'", "''") +
        "\'," + QString::number(comboTypOGUG->currentIndex()) + "," +
        QString::number(comboTypTrFl->currentIndex()) + ",\'" +
        tableWidget_Hefe->item(i, TableHefeColTemperatur)
            ->text()
            .replace("'", "''") +
        "\',\'" +
        tableWidget_Hefe->item(i, TableHefeColEigenschaften)
            ->text()
            .replace("'", "''") +
  ...
  for (int i = 0; i < tableWidget_WeitereZutaten->rowCount(); i++) {
    QDoubleSpinBox *dsbMenge =
        (QDoubleSpinBox *)tableWidget_WeitereZutaten->cellWidget(
            i, TableWZutatColMenge);
    QComboBox *comboEinheit =
        (QComboBox *)tableWidget_WeitereZutaten->cellWidget(
            i, TableWZutatColEinheit);
    QComboBox *comboTyp = (QComboBox *)tableWidget_WeitereZutaten->cellWidget(
        i, TableWZutatColTyp);
    QDoubleSpinBox *dsbAusbeute =
        (QDoubleSpinBox *)tableWidget_WeitereZutaten->cellWidget(
            i, TableWZutatColAusbeute);
    QDoubleSpinBox *dsbEBC =
        (QDoubleSpinBox *)tableWidget_WeitereZutaten->cellWidget(
            i, TableWZutatColFarbe);
    QDoubleSpinBox *dsbPreis =
        (QDoubleSpinBox *)tableWidget_WeitereZutaten->cellWidget(
            i, TableWZutatColPreis);
    QDateEdit *deEinlagerung =
        (QDateEdit *)tableWidget_WeitereZutaten->cellWidget(
            i, TableWZutatColEinlagerung);
    QDateEdit *deMhd = (QDateEdit *)tableWidget_WeitereZutaten->cellWidget(
        i, TableWZutatColMindesthalbat);
    sql = "INSERT INTO WeitereZutaten(Beschreibung, Menge, Einheiten, Typ, "
          "Ausbeute, EBC, Preis, Bemerkung, Eingelagert, Mindesthaltbar, Link) "
          "VALUES(\'" +
          tableWidget_WeitereZutaten->item(i, TableWZutatColName)
              ->text()
              .replace("'", "''") +
          ...
          tableWidget_WeitereZutaten->item(i, TableWZutatColBemerkung)
              ->text()
              .replace("'", "''") +
          "\','" + deEinlagerung->date().toString(Qt::ISODate) + "','" +
          deMhd->date().toString(Qt::ISODate) + "','" +
          tableWidget_WeitereZutaten->item(i, TableWZutatColLink)
              ->text()
              .replace("'", "''") +
          "')";
    if (!query.exec(sql)) {
  ...
  // Calcium
  sql = "UPDATE 'Wasser' SET 'Calcium'=" +
        QString::number(SpinBox_wwCalcium_mg->value());
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
    errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
  ...
  sql = "UPDATE 'Wasser' SET 'Magnesium'=" +
        QString::number(SpinBox_wwMagnesium_mg->value());
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
    errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
  ...
  sql = "UPDATE 'Wasser' SET 'Saeurekapazitaet'=" +
        QString::number(SpinBox_wwSaeurekapazitaet_mmol->value());
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
    errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
...
void MainWindowImpl::LeseRohstoffeDB() {
  ...
  if (!query.exec(sql)) {
    ...
    while (query.next())
      MalzNeueZeile(
          query.value(query.record().indexOf("Beschreibung")).toString(),
          ...
          QDate::fromString(
              query.value(query.record().indexOf("Eingelagert")).toString(),
              Qt::ISODate),
          QDate::fromString(
              query.value(query.record().indexOf("Mindesthaltbar")).toString(),
              Qt::ISODate),
  ...
  if (!query.exec(sql)) {
    ...
    SpinBox_wwCalcium_mg->setValue(
        query.value(query.record().indexOf("Calcium")).toDouble());
    SpinBox_wwMagnesium_mg->setValue(
        query.value(query.record().indexOf("Magnesium")).toDouble());
    SpinBox_wwSaeurekapazitaet_mmol->setValue(
        query.value(query.record().indexOf("Saeurekapazitaet")).toDouble());
  }
}
...
void MainWindowImpl::on_pushButton_MalzDel_clicked() {
  ...
  // Rohstoffname
  QString del_name =
      tableWidget_Malz->item(tableWidget_Malz->currentRow(), TableMalzColName)
          ->text();
  ...
  if (!query.exec(sql)) {
    ...
    while (query.next()) {
      if (ok) {
        ...
        QString sql2 = "SELECT * FROM Malzschuettung WHERE Name='" +
                       del_name.replace("'", "''") + "' AND SudID=" + sudid;
        if (!query2.exec(sql2)) {
          // Fehlermeldung Datenbankabfrage
          ErrorMessage *errorMessage = new ErrorMessage();
          errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
          ...
          if ((ok) && (query2.first())) {
            ...
            // Austauschen
            if (ok) {
              QString sql2 = "UPDATE Malzschuettung SET Name='" +
                             letzeAuswahl.replace("'", "''") +
                             "' WHERE Name='" + del_name +
                             "' AND SudID=" + sudid;
              if (!query2.exec(sql2)) {
                // Fehlermeldung Datenbankabfrage
...
void MainWindowImpl::on_pushButton_HopfenDel_clicked() {
  ...
  QString del_name =
      tableWidget_Hopfen
          ->item(tableWidget_Hopfen->currentRow(), TableHopfenColName)
          ->text();
  ...
  if (!query.exec(sql)) {
    ...
    while (query.next()) {
      if (ok) {
        ...
        QString sql2 = "SELECT * FROM WeitereZutatenGaben WHERE Name='" +
                       del_name.replace("'", "''") + "' AND SudID=" + sudid;
        if (!query2.exec(sql2)) {
          // Fehlermeldung Datenbankabfrage
          ErrorMessage *errorMessage = new ErrorMessage();
          ...
          if ((ok) && (query2.first())) {
            ...
            // Austauschen
            if (ok) {
              QString sql2 = "UPDATE WeitereZutatenGaben SET Name='" +
                             letzeAuswahl + "' WHERE Name='" + del_name +
                             "' AND SudID=" + sudid;
              if (!query2.exec(sql2)) {
                // Fehlermeldung Datenbankabfrage
                ErrorMessage *errorMessage = new ErrorMessage();
        ...
        sql2 = "SELECT * FROM Hopfengaben WHERE Name='" +
               del_name.replace("'", "''") + "' AND SudID=" + sudid;
        if (!query2.exec(sql2)) {
          // Fehlermeldung Datenbankabfrage
          ErrorMessage *errorMessage = new ErrorMessage();
          ...
          if ((ok) && (query2.first())) {
            ...
            // Austauschen
            if (ok) {
              QString sql2 = "UPDATE Hopfengaben SET Name='" +
                             letzeAuswahl.replace("'", "''") +
                             "' WHERE Name='" + del_name +
                             "' AND SudID=" + sudid;
              if (!query2.exec(sql2)) {
...
void MainWindowImpl::on_pushButton_HefeDel_clicked() {
  ...
  // Rohstoffname
  QString del_name =
      tableWidget_Hefe->item(tableWidget_Hefe->currentRow(), TableHefeColName)
          ->text();
    ...
    while (query.next()) {
      if (ok) {
        ...
        if (del_name == query.value(FeldNr).toString()) {
          ...
          // Austauschen
          if (ok) {
            QString sql2 = "UPDATE Sud SET AuswahlHefe= '" + letzeAuswahl +
                           "' WHERE  ID= " + sudid;
            if (!query2.exec(sql2)) {
              // Fehlermeldung Datenbankabfrage
              ErrorMessage *errorMessage = new ErrorMessage();
              ...
              // Berechneten wert nun nicht mehr stimmen.
              QString sql2 =
                  "UPDATE Sud SET NeuBerechnen= 1 WHERE  ID= " + sudid;
              if (!query2.exec(sql2)) {
                // Fehlermeldung Datenbankabfrage
                ErrorMessage *errorMessage = new ErrorMessage();
                errorMessage->showMessage(
...
void MainWindowImpl::SchreibeKonfig() {
  ...
  for (int i = 0; i < tableWidget_Sudauswahl->columnCount(); ++i)
    settings.setValue(
        "tableWidget_Sudauswahl_col" + QString::number(i),
        tableWidget_Sudauswahl->horizontalHeader()->sectionSize(i));
  for (int i = 0; i < tableWidget_Brauuebersicht->columnCount(); ++i)
    settings.setValue(
        "tableWidget_Brauuebersicht_col" + QString::number(i),
        tableWidget_Brauuebersicht->horizontalHeader()->sectionSize(i));
  for (int i = 0; i < tableWidget_Malz->columnCount(); ++i)
    settings.setValue("tableWidget_Malz_col" + QString::number(i),
                      tableWidget_Malz->horizontalHeader()->sectionSize(i));
  for (int i = 0; i < tableWidget_Hopfen->columnCount(); ++i)
    settings.setValue("tableWidget_Hopfen_col" + QString::number(i),
                      tableWidget_Hopfen->horizontalHeader()->sectionSize(i));
  for (int i = 0; i < tableWidget_Hefe->columnCount(); ++i)
    settings.setValue("tableWidget_Hefe_col" + QString::number(i),
                      tableWidget_Hefe->horizontalHeader()->sectionSize(i));
  for (int i = 0; i < tableWidget_WeitereZutaten->columnCount(); ++i)
    settings.setValue(
        "tableWidget_WeitereZutaten_col" + QString::number(i),
        tableWidget_WeitereZutaten->horizontalHeader()->sectionSize(i));
  settings.endGroup();
  ...
  settings.setValue("FilterNichtGebraut",
                    radioButton_FilterNichtGebraut->isChecked());
  ...
  settings.setValue("MerklisteMengenEinbeziehen",
                    checkBox_MerklisteMengen->checkState());
  settings.endGroup();
}
...
void MainWindowImpl::LeseKonfig() {
  ...
  radioButton_FilterNichtGebraut->setChecked(
      settings.value("FilterNichtGebraut").toBool());
  radioButton_FilterGebrautNichtAbgefuellt->setChecked(
  ...
  // Einstellungen Sonstiges
  settings.beginGroup("sonstiges");
  checkBox_MerklisteMengen->setChecked(
      settings.value("MerklisteMengenEinbeziehen", false).toBool());
  settings.endGroup();
}
...
void MainWindowImpl::createActions() {
  ...
  connect(schuettungProzent, SIGNAL(triggered()), this,
          SLOT(slot_schuettungProzent()));
  ...
  connect(EntsperreEingabefelder, SIGNAL(triggered()), this,
          SLOT(slot_EntsperreEingabefelder()));

  // Setzt das Bit BierGebraut zurück
  ResetBierGebraut = new QAction("", this);
  connect(ResetBierGebraut, SIGNAL(triggered()), this,
          SLOT(slot_ResetBierWurdeGebraut()));

  // Setzt das Bit Abgefuellt zurück
  ResetAbgefuellt = new QAction("", this);
  connect(ResetAbgefuellt, SIGNAL(triggered()), this,
          SLOT(slot_ResetAbgefuellt()));

  // Setzt das Bit Abgefuellt zurück
  ResetVerbraucht = new QAction("", this);
  connect(ResetVerbraucht, SIGNAL(triggered()), this,
          SLOT(slot_ResetBierVerbraucht()));

  // Setzt den Zugabestatus der Weiteren Zutaten zurück
  ResetZugabestatus = new QAction("", this);
  connect(ResetZugabestatus, SIGNAL(triggered()), this,
          SLOT(slot_ResetWZZugabestatus()));
}
...
void MainWindowImpl::ErstelleSprachMenu() {
  ...
  connect(langGroup, SIGNAL(triggered(QAction *)), this,
          SLOT(slot_SpracheWechselt(QAction *)));
...
void MainWindowImpl::SchreibeSuddatenDB() {
  ...
  sql += "Kommentar='";
  sql +=
      textEdit_Kommentar->document()->toPlainText().replace("'", "''") + "', ";
  // Braudatum
  sql += "Braudatum='";
  ...
  sql += QString::number(spinBox_JungbiermengeAbfuellen->value() +
                         spinBox_SpeisemengeGesammt->value() / 1000) +
         "', ";
...
void MainWindowImpl::SchreibeBewertungenDB() {
  ...
  QSqlQuery query;
  QString sql =
      "DELETE FROM Bewertungen WHERE SudID =" + QString::number(AktuelleSudID);
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
    errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
  ...
  for (int i = 0; i < list_Bewertung.count(); i++) {
    sql = "INSERT INTO Bewertungen(SudID, Woche, Datum, Sterne, Bemerkung, "
          "Farbe,";
    sql += "FarbeBemerkung, Schaum, SchaumBemerkung, Geruch, GeruchBemerkung, "
           "Geschmack, GeschmackBemerkung,";
    sql += "Antrunk, AntrunkBemerkung, Haupttrunk, HaupttrunkBemerkung, "
           "Nachtrunk, NachtrunkBemerkung,";
...
void MainWindowImpl::LeseSuddatenDB() {
  ...
  int FeldNr;
  QString sql =
      "SELECT * FROM Sud WHERE ID=" + QString::number(AktuelleSudID) + ";";
  if (!query_sud.exec(sql)) {
    ...
    if (query_sud.first()) {
      ...
      dateEdit_Braudatum->setDate(
          QDate::fromString(query_sud.value(FeldNr).toString(), Qt::ISODate));
      // Flag ob Bier gebraut wurde
      ...
      if (BierWurdeGebraut) {
        ...
        comboBox_AuswahlBrauanlage->setCurrentIndex(
            comboBox_AuswahlBrauanlage->count() - 1);
      } else {
      ...
      if (BierWurdeGebraut) {
        ...
        comboBox_AuswahlHefe->setCurrentIndex(comboBox_AuswahlHefe->count() -
                                              1);
      } else {
      ...
      spinBox_WuerzemengeVorHopfenseihen->setValue(
          query_sud.value(FeldNr).toDouble());
      // Stammwürze vor dem Hopfenseihen
      ...
      spinBox_NachisomerisierungsZeit->setValue(
          query_sud.value(FeldNr).toDouble());

      // Gewünschte Restalkalität
      FeldNr = query_sud.record().indexOf("RestalkalitaetSoll");
      SpinBox_waSollRestalkalitaet_dh->setValue(
          query_sud.value(FeldNr).toDouble());

      // Schnellgärprobe aktiv
      FeldNr = query_sud.record().indexOf("SchnellgaerprobeAktiv");
      checkBox_SchnellgaerprobeAktiv->setChecked(
          query_sud.value(FeldNr).toBool());
      ...
      spinBox_JungbiermengeAbfuellen->setValue(
          query_sud.value(FeldNr).toDouble());
      ...
      if (BierWurdeGebraut) {
        ...
        // spinBox_Sudhausausbeute ->
        // setValue(query_sud.value(FeldNr).toDouble());
        ...
        // doubleSpinBox_WNachguss ->
        // setValue(query_sud.value(FeldNr).toDouble());
      }

      // Rasten Abfragen
      QSqlQuery query_rasten;
      sql = "SELECT * FROM Rasten WHERE SudID=" +
            QString::number(AktuelleSudID) + ";";
      if (!query_rasten.exec(sql)) {
        ...
        while (query_rasten.next()) {
          ...
          if (b) {
            ...
            connect(rast, SIGNAL(sig_vorClose(int)), this,
                    SLOT(slot_rastClose(int)));
            connect(rast, SIGNAL(sig_aenderung(int)), this,
                    SLOT(slot_rastAenderung(int)));
            connect(rast, SIGNAL(sig_nachOben(int)), this,
                    SLOT(slot_pushButton_RastNachOben(int)));
            connect(rast, SIGNAL(sig_nachUnten(int)), this,
                    SLOT(slot_pushButton_RastNachUnten(int)));
          }
          i++;
        }
      }
      ...
      sql = "SELECT * FROM Malzschuettung WHERE SudID=" +
            QString::number(AktuelleSudID) + " ORDER by Prozent DESC;";
      ...
      sql = "SELECT * FROM Hopfengaben WHERE SudID=" +
            QString::number(AktuelleSudID) +
            " AND Aktiv=1 ORDER by Vorderwuerze DESC, Zeit DESC;";
      if (!query_Hopfen.exec(sql)) {
        ...
        while (query_Hopfen.next()) {
          ...
          int FeldNr_Vorderwuerze =
              query_Hopfen.record().indexOf("Vorderwuerze");
      ...
      QSqlQuery query_ewz;
      sql = "SELECT * FROM WeitereZutatenGaben WHERE SudID=" +
            QString::number(AktuelleSudID) + " ORDER BY Zeitpunkt;";
      if (!query_ewz.exec(sql)) {
        ...
        while (query_ewz.next()) {
          ...
          connect(ewz, SIGNAL(sig_vorClose(int)), this,
                  SLOT(slot_ewzClose(int)));
          connect(ewz, SIGNAL(sig_getEwzTyp(QString)), this,
                  SLOT(slot_getEwzTyp(QString)));
          connect(ewz, SIGNAL(sig_getEwzEinheit(QString)), this,
                  SLOT(slot_getEwzEinheit(QString)));
          connect(ewz, SIGNAL(sig_getEwzAusbeute(QString)), this,
                  SLOT(slot_getEwzAusbeute(QString)));
          connect(ewz, SIGNAL(sig_getEwzFarbe(QString)), this,
                  SLOT(slot_getEwzFarbe(QString)));
          connect(ewz, SIGNAL(sig_getEwzPreis(QString)), this,
                  SLOT(slot_getEwzPreis(QString)));
          connect(ewz, SIGNAL(sig_getEwzPreisHopfen(QString)), this,
                  SLOT(slot_getEwzPreisHopfen(QString)));
          connect(ewz, SIGNAL(sig_zugeben(QString, int, double)), this,
                  SLOT(slot_EwzZugegeben(QString, int, double)));
          connect(ewz, SIGNAL(sig_getHopfenMenge(QString)), this,
          ...
          connect(ewz, SIGNAL(sig_Aenderung()), this,
                  SLOT(slot_EwzAenderung()));
          ...
          date_von = QDate::fromString(query_ewz.value(FeldNr).toString(),
                                       Qt::ISODate);
          QDate date_bis;
          FeldNr = query_ewz.record().indexOf("Zeitpunkt_bis");
          date_bis = QDate::fromString(query_ewz.value(FeldNr).toString(),
                                       Qt::ISODate);
      ...
      // Bewertungen abfragen
      QSqlQuery query_bew;
      sql = "SELECT * FROM Bewertungen WHERE SudID=" +
            QString::number(AktuelleSudID) + " ORDER BY Datum;";
      ;
        ...
        while (query_bew.next()) {
          ...
          if (!queryN.exec(sqlN)) {
            ...
            if (queryN.first()) {
              ...
              date = QDate::fromString(queryN.value(FeldNr).toString(),
                                       Qt::ISODate);
            }
          }
          ...
          connect(bew, SIGNAL(sig_wocheClicked(int)), this,
                  SLOT(slot_BewertungWoche_clicked(int)));
          connect(bew, SIGNAL(sig_vorClose(int)), this,
                  SLOT(slot_bewClose(int)));

          int FeldNr_Name = query_bew.record().indexOf("Datum");
          bew->setBewertungsdatum(QDate::fromString(
              query_bew.value(FeldNr_Name).toString(), Qt::ISODate));
          ...
          bew->setGesamteindruckBemerkung(
              query_bew.value(FeldNr_Name).toString());
...
void MainWindowImpl::FuelleRezeptComboAuswahlen() {
  ...
  if (!BierWurdeGebraut) {
    ...
    for (int i = 0; i < listWidget_Brauanlagen->count(); i++) {
      comboBox_AuswahlBrauanlage->addItem(
          listWidget_Brauanlagen->item(i)->text());
    }
...
void MainWindowImpl::BerAlles() {
  ...
  if (Gestartet && !AmLaden && !NichtBerechnen) {
    ...
    if (!BierWurdeGebraut) {
      ...
      // wenn nicht abgefüllt wurde comboliste für weitere zutaten zugeben in
      // hauptgärung füllen
      ...
      // Wenn die gespeicherte Bierfarbe 0 ist versuchen die Bierfarbe zu
      // berechnen
      ...
      if (spinBox_Preis->value() == 0) {
        ...
        spinBox_Preis->setPalette(
            spinBox_Haushaltszucker500mlFlasche->palette());
      }
    }
...
void MainWindowImpl::slot_RohstoffMengeValueChanged(double menge) {
  ...
  if (prop.isValid()) {
    QTableWidgetItem *item =
        static_cast<QTableWidgetItem *>(prop.value<void *>());
    if (item) {
...
void MainWindowImpl::slot_RohstoffHaltbarValueChanged(const QDate &date) {
  ...
  if (dateedit) {
    QColor color = QDate::currentDate().daysTo(date) > 0
                       ? qApp->palette().color(QPalette::Active, QPalette::Base)
                       : QColor::fromRgb(200, 80, 80);
  ...
  if (spinbox) {
    ...
    if (ebc > 0)
      palette.setColor(QPalette::Active, QPalette::Base,
                       Berechnungen.GetFarbwert(ebc));
    else
      palette.setColor(QPalette::Active, QPalette::Base,
                       qApp->palette().color(QPalette::Active, QPalette::Base));
    palette.setColor(QPalette::Active, QPalette::Text,
                     ebc > 35 ? Qt::white : Qt::black);
...
void MainWindowImpl::slot_tableSyncValueChanged(double value) {
  ...
  QVariant prop = QObject::sender()->property("TableWidgetItem");
  if (prop.isValid()) {
    MyTableWidgetItemNumeric *item =
        static_cast<MyTableWidgetItemNumeric *>(prop.value<void *>());
    if (item)
      item->setValue(value);
  }
}
...
void MainWindowImpl::BerSchuettung() {
  ...
  double gs;
  gs = Berechnungen.GetGesammtSchuettung(
...
void MainWindowImpl::BerHopfen() {
  ...
  for (int o = 0; o < list_Hopfengaben.count(); o++) {
    ...
    Kochzeiten[o] = list_Hopfengaben[o]->getKochzeit() +
                    spinBox_NachisomerisierungsZeit->value();
    ...
    for (int i = 0; i < tableWidget_Hopfen->rowCount(); i++) {
      if (s == tableWidget_Hopfen->item(i, TableHopfenColName)->text()) {
        QDoubleSpinBox *spinBox =
            (QDoubleSpinBox *)tableWidget_Hopfen->cellWidget(
                i, TableHopfenColAlpha);
        Alphaprozent[o] = spinBox->value();
        Pellets[o] =
            tableWidget_Hopfen->item(i, TableHopfenColPellets)->checkState();
  ...
  if (!berIBUProzent) {
    Berechnungen.BerHopfenGewichtProzent(
        HopfenMengen, Alphaprozent, MengenProzent, Kochzeiten, Pellets,
        spinBox_IBU->value() * highGravityFaktor, list_Hopfengaben.count(), vwh,
        Ausbeute, IBUAnteil);
  }
  // Berechnen nach IBU prozent
  else {
    Berechnungen.BerHopfenIBUProzent(
        HopfenMengen, Alphaprozent, MengenProzent, Kochzeiten, Pellets,
        spinBox_IBU->value() * highGravityFaktor, list_Hopfengaben.count(), vwh,
        Ausbeute, IBUAnteil);
  }
...
void MainWindowImpl::LeseGeraetelisteDB(int id) {
  ...
  QString sql =
      "SELECT Bezeichnung FROM Geraete WHERE AusruestungAnlagenID = " +
      QString::number(id);
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
...
void MainWindowImpl::SchreibeGeraetelisteDB(int id) {
  if (AenderungGeraeteliste) {
    ...
    // Zuerst alle Einträge in der Tabelle löschen
    QString sql = "DELETE FROM Geraete WHERE AusruestungAnlagenID = " +
                  QString::number(id);
    if (!query.exec(sql)) {
      // Fehlermeldung Datenbankabfrage
      ErrorMessage *errorMessage = new ErrorMessage();
...
void MainWindowImpl::BerBraudaten() {
  ...
  // Anzeigefelder der stammwürze ausblenden wenn weitere zutaten keine rolle
  // spielen
  if (sw_ewz == 0) {
  ...
  double sw_ewz_kochen =
      sw_ewz - (sw_gesammt - (sw_kochen / highGravityFaktor));

  // Grünschlauchzeitpunkt
  spinBox_Gruenschlauchzeitpunkt->setValue(
      Berechnungen.GetGruenschlauchzeitpunkt(
          doubleSpinBox_CO2->value(), spinBox_TemperaturJungbier->value(),
          SWSchnellgaerprobe));

  // Erforderliche Speisemenge
  spinBox_SpeisemengeGesammt->setValue(Berechnungen.GetSpeiseGesammt(
      spinBox_SWAnstellen->value(), spinBox_SWJungbier->value(),
      spinBox_JungbiermengeAbfuellen->value(), spinBox_Speisemenge->value(),
      checkBox_Spunden->isChecked()));

  // Abgefuellte Biermenge
  spinBox_BiermengeAbfuellen->setValue(spinBox_JungbiermengeAbfuellen->value() +
                                       spinBox_SpeisemengeGesammt->value() /
                                           1000);

  // Erforderliche Zuckergabe gesammt
  spinBox_HaushaltszuckerGesammt->setValue(
      Berechnungen.GetHaushaltszuckerGesammt());

  // Erforderliche Zuckergabe gesammt
  spinBox_TraubenzuckerGesammt->setValue(
      Berechnungen.GetHaushaltszuckerGesammt() * 1.16);

  // Erforderliche Speisemenge pro 0,5 Liter Flasche
  spinBox_Speisemenge500mlFlasche->setValue(
      spinBox_SpeisemengeGesammt->value() /
      spinBox_JungbiermengeAbfuellen->value() / 2);

  // Erforderliche Zuckergabe pro 0,5 Liter Flasche
  spinBox_Haushaltszucker500mlFlasche->setValue(
      spinBox_HaushaltszuckerGesammt->value() /
      spinBox_JungbiermengeAbfuellen->value() / 2);

  // Erforderliche Traubenzuckergabe pro 0,5 Liter Flasche
  spinBox_Traubenzucker500mlFlasche->setValue(
      spinBox_HaushaltszuckerGesammt->value() /
      spinBox_JungbiermengeAbfuellen->value() / 2 * 1.16);

  // Erforderliche Speisemenge pro 0,33 Liter Flasche
  spinBox_Speisemenge333mlFlasche->setValue(
      spinBox_SpeisemengeGesammt->value() /
      spinBox_JungbiermengeAbfuellen->value() / 3);

  // Erforderliche Zuckergabe pro 0,33 Liter Flasche
  spinBox_Haushaltszucker333mlFlasche->setValue(
      spinBox_HaushaltszuckerGesammt->value() /
      spinBox_JungbiermengeAbfuellen->value() / 3);

  // Erforderliche Zuckergabe pro 0,33 Liter Flasche
  spinBox_Traubenzucker333mlFlasche->setValue(
      spinBox_HaushaltszuckerGesammt->value() /
      spinBox_JungbiermengeAbfuellen->value() / 3 * 1.16);

  // Alkoholgehalt
  double sw = spinBox_SWAnstellen->value();
  sw = sw + sw_ewz_gaerung;
  sw += (spinBox_HaushaltszuckerGesammt->value() / 10) /
        spinBox_WuerzemengeAnstellen->value();
  spinBox_AlkoholVol->setValue(
      Berechnungen.BerAlkohoVol(sw, SWSchnellgaerprobe));

  // Scheinbarer EVG
  sw = spinBox_SWAnstellen->value();
  spinBox_SEVG->setValue(
      Berechnungen.GetScheinbarerEVG(sw, SWSchnellgaerprobe));
  // Tatsächlicher EVG
  spinBox_TEVG->setValue(
      Berechnungen.GetTatsaechlicherEVG(sw, SWSchnellgaerprobe));
  ...
  spinBox_AusbeuteEffektiv2->setValue(Berechnungen.GetSudhausausbeute(
      spinBox_SWAnstellen->value() * highGravityFaktor - sw_ewz_kochen,
      (spinBox_WuerzemengeAnstellen->value() + spinBox_Speisemenge->value()) /
          highGravityFaktor,
      doubleSpinBox_S_Gesammt->value()));

  // Sudhausausbeute
  spinBox_Sudhausausbeute->setValue(Berechnungen.GetSudhausausbeute(
      spinBox_SWKochende->value() - sw_ewz_kochen,
      spinBox_WuerzemengeVorHopfenseihen->value(),
      doubleSpinBox_S_Gesammt->value()));

  // Würzemenge bei Kochbeginn
  spinBox_MengeSollKochbegin->setValue(
      doubleSpinBox_VolumenPfannevoll->value());
  // Füllhöhe von unten
  double Grundflaeche = pow(getSudpfanneDurchmesser() / 2, 2) * M_PI;
  spinBox_MengeSollcmVomBoden->setValue(spinBox_MengeSollKochbegin->value() *
                                        1000 / Grundflaeche);
  // Füllhöhe von oben
  spinBox_MengeSollcmVonOben->setValue(getSudpfanneHoehe() -
                                       spinBox_MengeSollcmVomBoden->value());
  // Stamwürze bei Kochbeginn
  double swvsoll = spinBox_SWSollKochen->value();
  double mengeKochende100grad =
      Berechnungen.BerVolumenWasser(20, 99, spinBox_Menge->value());
  swvsoll =
      swvsoll / doubleSpinBox_VolumenPfannevoll->value() * mengeKochende100grad;
...
void MainWindowImpl::slot_pushButton_gebraut() {
  ...
  msgBox.setInformativeText("");
  msgBox.setText(
      trUtf8("Sollen die verwendeten Rohstoffe vom Bestand abgezogen "
             "werden?\n\nHinweis: Zutaten, die bei der Gärung hinzugegeben "
             "werden, werden jetzt nicht verrechnet."));
  // msgBox.setInformativeText(trUtf8("Sollen die verwendeten Rohstoffe vom
  // Bestand abgezogen werden?"));
  msgBox.setIcon(QMessageBox::Question);
  // msgBox.setDefaultButton(QMessageBox::Save);
  QPushButton *JaButton =
      msgBox.addButton(trUtf8("Ja"), QMessageBox::ActionRole);
  ...
  doubleSpinBox_SWSchnellgaerprobe->setValue(spinBox_SWAnstellen->value() +
                                             sw_ewz_gaerung);
  doubleSpinBox_SWHauptgaerprobe->setValue(spinBox_SWAnstellen->value() +
                                           sw_ewz_gaerung);
  ...
  for (int i = 0; i < list_Malzgaben.count(); i++) {
    ...
    for (int o = 0; o < AnzahlMalzEintraege; o++) {
      ...
      if (tableWidget_Malz->item(o, TableMalzColName)->text() == s) {
        QDoubleSpinBox *dsbMenge =
            (QDoubleSpinBox *)tableWidget_Malz->cellWidget(o,
                                                           TableMalzColMenge);
        d = dsbMenge->value();
    ...
    if (list_EwZutat[i]->getZeitpunkt() != EWZ_Zeitpunkt_Gaerung) {
      ...
      if (list_EwZutat[i]->getTyp() != EWZ_Typ_Hopfen) {
        ...
        for (int o = 0; o < tableWidget_WeitereZutaten->rowCount(); o++) {
          // wenn eintrag übereinstimmt
          if (tableWidget_WeitereZutaten->item(o, TableWZutatColName)->text() ==
              s) {
            QDoubleSpinBox *dsbMenge =
                (QDoubleSpinBox *)tableWidget_WeitereZutaten->cellWidget(
                    o, TableWZutatColMenge);
            if (list_EwZutat[i]->getEinheit() == EWZ_Einheit_Kg)
              dsbMenge->setValue(dsbMenge->value() -
                                 (list_EwZutat[i]->getErg_Menge() / 1000));
            else
              dsbMenge->setValue(dsbMenge->value() -
                                 list_EwZutat[i]->getErg_Menge());
          }
        }
      }
      // Wenn Hopfen
...
void MainWindowImpl::slot_pushButton_SudAbgefuellt() {
  if (BierWurdeGebraut) {
    //überprüfen ob noch zutaten für die gärung nicht zugegeben oder noch nicht
    // entnommen wurden
    bool gefunden = false;
    for (int i = 0; i < list_EwZutat.count(); i++) {
      if (list_EwZutat[i]->getZeitpunkt() == EWZ_Zeitpunkt_Gaerung) {
        ...
        } else if (list_EwZutat[i]->getZugabestatus() ==
                       EWZ_Zugabestatus_Zugegeben &&
    ...
    // Es sind noch zutaen vorhanden wo der zugabe oder entnahmezustand nicht
    // definiert wurden
    if (gefunden) {
      QMessageBox msgBox;
...
void MainWindowImpl::CheckRohstoffeVorhanden() {
  ...
  for (int i = 0; i < list_Malzgaben.count(); i++) {
    ...
    // Wenn angehakt Sude aus der Merkliste abfragen und überprüfen ob da dieser
    // Rohstoff auch verwendet wird.
      ...
      if (!query.exec(sql)) {
        ...
        while (query.next()) {
          ...
          if (SudID != AktuelleSudID) {
            ...
            QString sql2 = "SELECT * FROM 'Malzschuettung' WHERE SudID=" +
                           QString::number(SudID) + " AND Name='" + s + "'";
            if (!query2.exec(sql2)) {
              // Fehlermeldung Datenbankabfrage
              ErrorMessage *errorMessage = new ErrorMessage();
              errorMessage->showMessage(
      ...
      if (tableWidget_Malz->item(o, TableMalzColName)->text() == s) {
        ...
        QDoubleSpinBox *dsbMenge =
            (QDoubleSpinBox *)tableWidget_Malz->cellWidget(o,
                                                           TableMalzColMenge);
        ist = dsbMenge->value();
        QDoubleSpinBox *dsbMaxprozent =
            (QDoubleSpinBox *)tableWidget_Malz->cellWidget(
                o, TableMalzColMaxSchuettung);
    ...
    if (soll > ist) {
      ...
      if (istprozent > maxprozent) {
        QString sf = trUtf8("Maximal empfohlener Schüttungsanteil (") +
                     QString::number(maxprozent) +
                     trUtf8("%) wurde überschritten");
        list_Malzgaben[i]->ergWidget->setToolTip(sf);
        list_Malzgaben[i]->ergWidget->icon_warnung->setVisible(true);
        list_Malzgaben[i]->ergWidget->icon_achtung->setVisible(false);
      } else {
        list_Malzgaben[i]->ergWidget->setToolTip(
            trUtf8("Rest ") + QString::number(ist - soll) + "kg");
        list_Malzgaben[i]->ergWidget->icon_achtung->setVisible(false);
        list_Malzgaben[i]->ergWidget->icon_warnung->setVisible(false);
      }
    }
  ...
  for (int i = 0; i < list_Hopfengaben.count(); i++) {
    ...
    for (int o = 0; o < list_EwZutat.count(); o++) {
      if ((list_EwZutat[o]->getName() == s) &&
          (list_EwZutat[o]->getTyp() == EWZ_Typ_Hopfen)) {
      ...
      if (!query.exec(sql)) {
        ...
        while (query.next()) {
          ...
          if (SudID != AktuelleSudID) {
            ...
            QString sql2 = "SELECT * FROM 'Hopfengaben' WHERE SudID=" +
                           QString::number(SudID) + " AND Aktiv=1 AND Name='" +
                           s + "'";
            if (!query2.exec(sql2)) {
              // Fehlermeldung Datenbankabfrage
              ErrorMessage *errorMessage = new ErrorMessage();
            ...
            sql2 = "SELECT * FROM 'WeitereZutatenGaben' WHERE SudID=" +
                   QString::number(SudID) + " AND Name='" + s + "'";
            if (!query2.exec(sql2)) {
              // Fehlermeldung Datenbankabfrage
              ErrorMessage *errorMessage = new ErrorMessage();
    ...
    if (soll > ist) {
      QString sf =
          QString::number(soll - ist) + trUtf8(" g zu wenig Hopfen vorhanden");
      list_Hopfengaben[i]->ergWidget->setToolTip(sf);
      list_Hopfengaben[i]->ergWidget->icon_achtung->setVisible(true);
    } else {
      list_Hopfengaben[i]->ergWidget->setToolTip(
          trUtf8("Rest ") + QString::number(ist - soll) + "g");
  ...
  if (s != "") {
    ...
    for (int i = 0; i < AnzahlHefeEintraege; i++) {
      ...
      if (tableWidget_Hefe->item(i, TableHefeColName)->text() == s) {
        QSpinBox *spinBox =
            (QSpinBox *)tableWidget_Hefe->cellWidget(i, TableHefeColMenge);
        ist = spinBox->value();
        // benötigte Hefemenge
    ...
    // Anzeige Einfärben wenn Rohstoff nicht vorrätig wäre
    if (soll > ist) {
      QString sf = QString::number(soll - ist) +
                   trUtf8(" Einheiten zu wenig Hefe vorhanden");
      widget_AnzahlHefeEinheiten->setToolTip(sf);
      icon_Hefe_achtung->setVisible(true);
    } else {
      widget_AnzahlHefeEinheiten->setToolTip(
          trUtf8("Rest ") + QString::number(ist - soll) + trUtf8("Einheiten"));
  ...
  for (int i = 0; i < list_EwZutat.count(); i++) {
    ...
    if (list_EwZutat[i]->getTyp() == EWZ_Typ_Hopfen) {
      ...
      // Nun überprüfen ob der Hopfen in den Weiteren Zutaten noch einmal
      // vorkommt
      for (int o = 0; o < list_EwZutat.count(); o++) {
        if ((list_EwZutat[o]->getName() == s) &&
            (list_EwZutat[o]->getTyp() == EWZ_Typ_Hopfen)) {
          // wenn der Eintrag der gleiche ist dann natürlich nicht
          // berücksichtigen
      ...
      // Wenn angehakt Sude aus der Merkliste abfragen und überprüfen ob da
      // dieser Rohstoff auch verwendet wird.
      if (checkBox_MerklisteMengen->isChecked()) {
        QSqlQuery query;
        QString sql;
        sql = "SELECT ID FROM 'Sud' WHERE MerklistenID=1";
        if (!query.exec(sql)) {
          ...
          while (query.next()) {
            ...
            if (SudID != AktuelleSudID) {
              ...
              // Rohstoffeintrag abfragen
              QString sql2 = "SELECT * FROM 'Hopfengaben' WHERE SudID=" +
                             QString::number(SudID) +
                             " AND Aktiv=1 AND Name='" + s + "'";
              if (!query2.exec(sql2)) {
                // Fehlermeldung Datenbankabfrage
                ErrorMessage *errorMessage = new ErrorMessage();
                errorMessage->showMessage(
    ...
    // alle anderen zutaten
      ...
      soll = list_EwZutat[i]->getErg_Menge();
      int id = list_EwZutat[i]->getID();
      // Nun überprüfen ob die zutat in den weiteren Zutaten noch einmal
      // vorkommt
      for (int o = 0; o < list_EwZutat.count(); o++) {
        if ((list_EwZutat[o]->getName() == s) &&
            (list_EwZutat[o]->getTyp() != EWZ_Typ_Hopfen)) {
        ...
        if (!query.exec(sql)) {
          ...
          while (query.next()) {
            ...
            if (SudID != AktuelleSudID) {
              ...
              QString sql2 =
                  "SELECT * FROM 'WeitereZutatenGaben' WHERE SudID=" +
                  QString::number(SudID) + " AND Name='" + s + "'";
              if (!query2.exec(sql2)) {
                // Fehlermeldung Datenbankabfrage
                ErrorMessage *errorMessage = new ErrorMessage();
    ...
    if (doubleSpinBox_VolumenMaische->value() > getMaischenMaxNutzvolumen()) {
      QString s = trUtf8("Kapazität Maischekessel um ") +
                  QString::number(doubleSpinBox_VolumenMaische->value() -
                                  getMaischenMaxNutzvolumen()) +
                  trUtf8(" Liter überschritten");
      ...
      QString s = trUtf8("noch ") +
                  QString::number(getMaischenMaxNutzvolumen() -
                                  doubleSpinBox_VolumenMaische->value()) +
                  trUtf8(" Liter Platz");
      doubleSpinBox_VolumenMaische->setToolTip(s);
      doubleSpinBox_VolumenMaische->setPalette(paletteN);
    }
...
void MainWindowImpl::CheckPfannevoll() {
  ...
  if (BierWurdeGebraut) {
    ...
    if (doubleSpinBox_VolumenPfannevoll->value() >
        getSudpfanneMaxNutzvolumen()) {
...
int MainWindowImpl::strippedID(const QString &fullFileName) {
  ...
  // ),fullFileName.right(fullFileName.length() - fullFileName.indexOf("=") -
  // 1));
  return fullFileName
      .right(fullFileName.length() - fullFileName.indexOf("=") - 1)
      .toInt();
}
...
bool MainWindowImpl::AbfrageSpeichern() {
  ...
  msgBox.setInformativeText(
      trUtf8("Sollen die Änderungen gespeichert werden?"));
  msgBox.setIcon(QMessageBox::Question);
...
void MainWindowImpl::AddHopfengabe(bool vwh, QString Name, int Zeit,
                                   double Menge, double erg_Menge, double Alpha,
                                   int Pellets) {
  // Hopfen hinzufügen
  // Zutatenobjekt hinzufügen
  ...
  connect(hopfen, SIGNAL(sig_getHopfenMenge(QString)), this,
          SLOT(slot_HopfenGetMenge(QString)));
...
void MainWindowImpl::slot_tabWidgetChanged(int) {
  // Wenn sich die Rohstoffe geändert haben müssen die comboauswahlen und
  // Erweiterte
...
void MainWindowImpl::on_pushButton_MalzKopie_clicked() {
  ...
  if (i >= 0 && i < tableWidget_Malz->rowCount()) {
    ...
    tableWidget_Malz->setSortingEnabled(false);
    MalzNeueZeile(
        tableWidget_Malz->item(i, TableMalzColName)->text() + trUtf8(" Kopie"),
        ((QDoubleSpinBox *)tableWidget_Malz->cellWidget(i, TableMalzColFarbe))
            ->value(),
        ((QDoubleSpinBox *)tableWidget_Malz->cellWidget(
             i, TableMalzColMaxSchuettung))
...
void MainWindowImpl::on_pushButton_HefeKopie_clicked() {
  ...
  if (i >= 0 && i < tableWidget_Hefe->rowCount()) {
    ...
    tableWidget_Hefe->setSortingEnabled(false);
    HefeNeueZeile(
        tableWidget_Hefe->item(i, TableHefeColName)->text() + trUtf8(" Kopie"),
        ((QDoubleSpinBox *)tableWidget_Hefe->cellWidget(i, TableHefeColMenge))
            ->value(),
        ((QDoubleSpinBox *)tableWidget_Hefe->cellWidget(
             i, TableHefeColWuerzemenge))
            ->value(),
        ((QDoubleSpinBox *)tableWidget_Hefe->cellWidget(i, TableHefeColPreis))
            ->value(),
        ...
        ((QComboBox *)tableWidget_Hefe->cellWidget(i,
...
void MainWindowImpl::on_pushButton_WeitereZutatenKopie_clicked() {
  ...
  if (i >= 0 && i < tableWidget_WeitereZutaten->rowCount()) {
    ...
    tableWidget_WeitereZutaten->setSortingEnabled(false);
    WeitereZutatNeueZeile(
        tableWidget_WeitereZutaten->item(i, TableWZutatColName)->text() +
            trUtf8(" Kopie"),
        ((QDoubleSpinBox *)tableWidget_WeitereZutaten->cellWidget(
             i, TableWZutatColMenge))
            ->value(),
        ((QComboBox *)tableWidget_WeitereZutaten->cellWidget(
             i, TableWZutatColEinheit))
            ->currentIndex(),
        ((QComboBox *)tableWidget_WeitereZutaten->cellWidget(i,
                                                             TableWZutatColTyp))
            ->currentIndex(),
        ((QDoubleSpinBox *)tableWidget_WeitereZutaten->cellWidget(
             i, TableWZutatColAusbeute))
...
void MainWindowImpl::BerFarbe(double cEBC) {
  ...
  if (cEBC == 0) {
    double *schuettung =
        new double[list_Malzgaben.count() + list_EwZutat.count()];
    ...
    // wenn nicht alle Farbwerte ermittelt werden konnten keinen Farbwert
    // berechnen
    double EBC;
    if (gefunden != list_Malzgaben.count() + anzahlWZAuswerten) {
      ...
      Bierfarbe = Berechnungen.GetFarbwert(
          schuettung, farbe, list_Malzgaben.count() + anzahlWZAuswerten,
          sw_schuettung, getKorrekturFarbe());
...
void MainWindowImpl::FuelleSudauswahl() {
  ...
  if (!query.exec(sql)) {
    ...
    while (query.next()) {
      ...
      bool gebraut =
          query.value(query.record().indexOf("BierWurdeGebraut")).toBool();
      bool abgefuellt =
...
void MainWindowImpl::slot_pushButton_SudKopie() {
  QString id =
      tableWidget_Sudauswahl->item(tableWidget_Sudauswahl->currentRow(), 0)
          ->text();
  QString name =
...
void MainWindowImpl::slot_pushButton_SudDel() {
  ...
  msgBox.setDefaultButton(
      msgBox.addButton(trUtf8("Abrechen"), QMessageBox::ActionRole));
...
void MainWindowImpl::AddMalzgabe(QString Name, double Prozent, double erg_Menge,
                                 double Farbe) {
  // Malz hinzufügen
  // Zutatenobjekt hinzufügen
  ...
  connect(malz, SIGNAL(sig_getMalzMenge(QString)), this,
          SLOT(slot_MalzGetMenge(QString)));
  // Zutatenliste füllen
...
void MainWindowImpl::SchreibeRastenDB() {
  QSqlQuery query;
  QString sql =
      "DELETE FROM Rasten WHERE SudID =" + QString::number(AktuelleSudID);
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
    errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
  ...
  for (int i = 0; i < list_Rasten.count(); i++) {
    sql = "INSERT INTO Rasten(SudID, RastAktiv, RastTemp, RastDauer, RastName) "
          "VALUES(" +
          QString::number(AktuelleSudID) + "," + QString::number(1) + "," +
...
void MainWindowImpl::SchreibeHopfengabenDB() {
  ...
  // Malzauswahl 1
  QString sql =
      "DELETE FROM Hopfengaben WHERE SudID=" + QString::number(AktuelleSudID);
  // Abfrage Abschicken
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
  ...
  for (int i = 0; i < list_Hopfengaben.count(); i++) {
    sql = "INSERT INTO Hopfengaben (SudID, Aktiv, Zeit, Prozent, "
          "erg_Hopfentext, erg_Menge, Alpha, Pellets, Vorderwuerze, Name)";
    // Prozentwert
    sql += " VALUES(";
    // SudID
    ...
    // Alpha Prozent und Pellets nur schreiben wenn nicht gebraut oder wert auf
    // 0 ist
    QString Alphaprozent = "0";
    QString Pellets = "0";
    if (!BierWurdeGebraut || (list_Hopfengaben[i]->getAlpha() == 0)) {
      for (int o = 0; o < tableWidget_Hopfen->rowCount(); o++) {
        if (list_Hopfengaben[i]->getName() ==
            tableWidget_Hopfen->item(o, TableHopfenColName)->text()) {
          QDoubleSpinBox *spinBox =
              (QDoubleSpinBox *)tableWidget_Hopfen->cellWidget(
                  o, TableHopfenColAlpha);
          Alphaprozent = QString::number(spinBox->value());
          bool b =
              tableWidget_Hopfen->item(o, TableHopfenColPellets)->checkState();
...
void MainWindowImpl::SchreibeMalzschuettungDB() {
  ...
  QSqlQuery query;
  // Malzauswahl 1
  QString sql = "DELETE FROM Malzschuettung WHERE SudID=" +
                QString::number(AktuelleSudID);
  // Abfrage Abschicken
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
  ...
  for (int i = 0; i < list_Malzgaben.count(); i++) {
    ...
    // Malzauswahl 1
    QString sql =
        "INSERT INTO Malzschuettung (SudID, Prozent, erg_Menge, Farbe, Name)";
    // Prozentwert
    sql += " VALUES(";
    sql += QString::number(AktuelleSudID) + ", ";
    ...
    if (!BierWurdeGebraut || (list_Malzgaben[i]->getFarbe() == 0)) {
      for (int i = 0; i < tableWidget_Malz->rowCount(); i++) {
        if (Name == tableWidget_Malz->item(i, TableMalzColName)->text()) {
          QDoubleSpinBox *dsbFarbe =
              (QDoubleSpinBox *)tableWidget_Malz->cellWidget(i,
                                                             TableMalzColFarbe);
...
void MainWindowImpl::BerKosten() {
  ...
  // wenn nicht alle preise gefunden wurden kann die Kostenrechnung nicht
  ...
  str = trUtf8("Schüttung: ") + QString::number(prozent) + "% " +
        QString::number(round(kostenSchuettung * 100) / 100) + trUtf8(" €") +
        "\n";
  prozent = round(kostenHopfen / summe * 100);
  str += trUtf8("Hopfen: ") + QString::number(prozent) + "% " +
         QString::number(round(kostenHopfen * 100) / 100) + trUtf8(" €") + "\n";
  prozent = round(kostenWeitereZutaten / summe * 100);
  str += trUtf8("Weitere Zutaten: ") + QString::number(prozent) + "% " +
         QString::number(round(kostenWeitereZutaten * 100) / 100) +
         trUtf8(" €") + "\n";
  prozent = round(kostenHefe / summe * 100);
  str += trUtf8("Hefe: ") + QString::number(prozent) + "% " +
         QString::number(round(kostenHefe * 100) / 100) + trUtf8(" €") + "\n";
  prozent = round(kostenSonstiges / summe * 100);
  str += trUtf8("Nebenkosten: ") + QString::number(prozent) + "% " +
         QString::number(round(kostenSonstiges * 100) / 100) + trUtf8(" €") +
         "\n";
      ...
      if (!query.exec(sql)) {
        ...
        while (query.next()) {
          ...
          QDateTime dt = QDateTime::fromString(
              query.value(FeldNr).toString() + "T12:00:00", Qt::ISODate);
...
void MainWindowImpl::BerEffektiveAusbeuteMittel() {
  ...
  sql = "SELECT erg_EffektiveAusbeute FROM Sud WHERE BierWurdeGebraut=1 AND "
        "AusbeuteIgnorieren=0 AND AuswahlBrauanlage=" +
        QString::number(AnlagenID);
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
    errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
...
void MainWindowImpl::on_pushButton_AddSchnellgaerMessung_clicked() {
  ...
  alc = Berechnungen.BerAlkohoVol(spinBox_SWSollGesammt->value(),
                                  doubleSpinBox_SWSchnellgaerprobe->value());
  ...
  // Wenn Bier noch nicht abgefüllt wurde dann in Wert in Brau & Gärdaten
  // übernehmen
  if (!BierWurdeAbgefuellt) {
...
void MainWindowImpl::SchreibeSchnellgaerverlaufDB() {
  ...
  // Zuerst alle Einträge in der Tabelle löschen
  QString sql = "DELETE FROM Schnellgaerverlauf WHERE SudID = " +
                QString::number(AktuelleSudID) + ";";
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
  ...
  for (int i = 0; i < tableWidget_Schnellgaerverlauf->rowCount(); i++) {
    QDateTime dt;
    dt = tableWidget_Schnellgaerverlauf->item(i, 0)
             ->data(Qt::DisplayRole)
             .toDateTime();
    sql = "INSERT INTO Schnellgaerverlauf(SudID, Zeitstempel, SW, Alc, Temp) "
          "VALUES(" +
          QString::number(AktuelleSudID) + "," + "'" +
...
void MainWindowImpl::LeseSchnellgaerverlaufDB() {
  ...
  // Tabelle einlesen
  QString sql = "SELECT * FROM Schnellgaerverlauf WHERE SudID=" +
                QString::number(AktuelleSudID) + " ORDER BY Zeitstempel;";
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
    errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
    ...
    while (query.next()) {
      ...
      QTableWidgetItem *newItem3 = new QTableWidgetItem("");
      QTableWidgetItem *newItem4 = new QTableWidgetItem("");
      tableWidget_Schnellgaerverlauf->setRowCount(
          tableWidget_Schnellgaerverlauf->rowCount() + 1);
      // Zeitstempel
      ...
      double alc =
          Berechnungen.BerAlkohoVol(spinBox_SWSollGesammt->value(), sw);
      alc = double(qRound(alc * 10)) / 10;
      newItem3->setData(Qt::DisplayRole, alc);
      newItem3->setFlags(newItem3->flags() & (~Qt::ItemIsEditable));
      tableWidget_Schnellgaerverlauf->setItem(i, 2, newItem3);
...
void MainWindowImpl::FuelleDiagrammSchnellgaerverlauf() {
  ...
  for (int i = 0; i < tableWidget_Schnellgaerverlauf->rowCount(); i++) {
    ...
    widget_DiaSchnellgaerverlauf->L1Daten.append(
        tableWidget_Schnellgaerverlauf->item(i, 1)
            ->data(Qt::DisplayRole)
            .toDouble());
    widget_DiaSchnellgaerverlauf->L2Daten.append(
        tableWidget_Schnellgaerverlauf->item(i, 2)
            ->data(Qt::DisplayRole)
            .toDouble());
    widget_DiaSchnellgaerverlauf->L3Daten.append(
        tableWidget_Schnellgaerverlauf->item(i, 3)
            ->data(Qt::DisplayRole)
            .toDouble());
  }
...
void MainWindowImpl::on_pushButton_DelSchnellgaerMessung_clicked() {
  tableWidget_Schnellgaerverlauf->removeRow(
      tableWidget_Schnellgaerverlauf->currentRow());
  FuelleDiagrammSchnellgaerverlauf();
  setAenderung(true);
}
...
void MainWindowImpl::SchreibeHauptgaerverlaufDB() {
  if (AenderungHauptgaerverlauf) {
    ...
    // Zuerst alle Einträge in der Tabelle löschen
    QString sql = "DELETE FROM Hauptgaerverlauf WHERE SudID = " +
                  QString::number(AktuelleSudID) + ";";
    if (!query.exec(sql)) {
      // Fehlermeldung Datenbankabfrage
      ErrorMessage *errorMessage = new ErrorMessage();
    ...
    for (int i = 0; i < tableWidget_Hauptgaerverlauf->rowCount(); i++) {
      QDateTime dt;
      dt = tableWidget_Hauptgaerverlauf->item(i, 0)
               ->data(Qt::DisplayRole)
               .toDateTime();
      sql = "INSERT INTO Hauptgaerverlauf(SudID, Zeitstempel, SW, Alc, Temp) "
            "VALUES(" +
            QString::number(AktuelleSudID) + "," + "'" +
            dt.toString(Qt::ISODate) + "'," +
...
void MainWindowImpl::LeseHauptgaerverlaufDB() {
  ...
  // Tabelle einlesen
  QString sql = "SELECT * FROM Hauptgaerverlauf WHERE SudID=" +
                QString::number(AktuelleSudID) + " ORDER BY Zeitstempel;";
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
    ...
    while (query.next()) {
      ...
      QTableWidgetItem *newItem3 = new QTableWidgetItem("");
      QTableWidgetItem *newItem4 = new QTableWidgetItem("");
      tableWidget_Hauptgaerverlauf->setRowCount(
          tableWidget_Hauptgaerverlauf->rowCount() + 1);
      // Zeitstempel
      FeldNr = query.record().indexOf("Zeitstempel");
      newItem1->setData(
          Qt::DisplayRole,
...
void MainWindowImpl::FuelleDiagrammHauptgaerverlauf() {
  ...
  for (int i = 0; i < tableWidget_Hauptgaerverlauf->rowCount(); i++) {
    ...
    widget_DiaHauptgaerverlauf->L1Daten.append(
        tableWidget_Hauptgaerverlauf->item(i, 1)
            ->data(Qt::DisplayRole)
            .toDouble());
    widget_DiaHauptgaerverlauf->L2Daten.append(
        tableWidget_Hauptgaerverlauf->item(i, 2)
            ->data(Qt::DisplayRole)
            .toDouble());
    widget_DiaHauptgaerverlauf->L3Daten.append(
        tableWidget_Hauptgaerverlauf->item(i, 3)
            ->data(Qt::DisplayRole)
            .toDouble());
  }
...
void MainWindowImpl::on_pushButton_DelHauptgaerMessung_clicked() {
  tableWidget_Hauptgaerverlauf->removeRow(
      tableWidget_Hauptgaerverlauf->currentRow());
  FuelleDiagrammHauptgaerverlauf();
  setAenderung(true);
  AenderungHauptgaerverlauf = true;
}
...
void MainWindowImpl::SchreibeNachgaerverlaufDB() {
  ...
  // Zuerst alle Einträge in der Tabelle löschen
  QString sql = "DELETE FROM Nachgaerverlauf WHERE SudID = " +
                QString::number(AktuelleSudID) + ";";
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
  ...
  for (int i = 0; i < tableWidget_Nachgaerverlauf->rowCount(); i++) {
    QDateTime dt;
    dt = tableWidget_Nachgaerverlauf->item(i, 0)
             ->data(Qt::DisplayRole)
             .toDateTime();
    sql = "INSERT INTO Nachgaerverlauf(SudID, Zeitstempel, Druck, Temp, CO2) "
          "VALUES(" +
          QString::number(AktuelleSudID) + "," + "'" +
          dt.toString(Qt::ISODate) + "'," +
...
void MainWindowImpl::LeseNachgaerverlaufDB() {
  ...
  // Tabelle einlesen
  QString sql = "SELECT * FROM Nachgaerverlauf WHERE SudID=" +
                QString::number(AktuelleSudID) + " ORDER BY Zeitstempel;";
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
    ...
    while (query.next()) {
      ...
      QTableWidgetItem *newItem3 = new QTableWidgetItem("");
      QTableWidgetItem *newItem4 = new QTableWidgetItem("");
      tableWidget_Nachgaerverlauf->setRowCount(
          tableWidget_Nachgaerverlauf->rowCount() + 1);
      // Zeitstempel
      FeldNr = query.record().indexOf("Zeitstempel");
      newItem1->setData(
          Qt::DisplayRole,
...
void MainWindowImpl::FuelleDiagrammNachgaerverlauf() {
  ...
  for (int i = 0; i < tableWidget_Nachgaerverlauf->rowCount(); i++) {
    ...
    widget_DiaNachgaerverlauf->L1Daten.append(
        tableWidget_Nachgaerverlauf->item(i, 3)
            ->data(Qt::DisplayRole)
            .toDouble());
    widget_DiaNachgaerverlauf->L2Daten.append(
        tableWidget_Nachgaerverlauf->item(i, 2)
            ->data(Qt::DisplayRole)
            .toDouble());
  }
...
void MainWindowImpl::on_pushButton_DelNachgaerMessung_clicked() {
  tableWidget_Nachgaerverlauf->removeRow(
      tableWidget_Nachgaerverlauf->currentRow());
  FuelleDiagrammNachgaerverlauf();
  setAenderung(true);
}
...
void MainWindowImpl::slot_schuettungProzent() {
  ...
  if (rdia.exec() == QDialog::Accepted && !BierWurdeGebraut) {
    ...
    if (rdia.comboBox_S_1->currentText() != "") {
      AddMalzgabe(rdia.comboBox_S_1->currentText(),
                  rdia.doubleSpinBox_Prozent_S_1->value(), double(0),
                  double(0));
    }
    if (rdia.comboBox_S_2->currentText() != "") {
      AddMalzgabe(rdia.comboBox_S_2->currentText(),
                  rdia.doubleSpinBox_Prozent_S_2->value(), double(0),
                  double(0));
    }
    if (rdia.comboBox_S_3->currentText() != "") {
      AddMalzgabe(rdia.comboBox_S_3->currentText(),
                  rdia.doubleSpinBox_Prozent_S_3->value(), double(0),
                  double(0));
    }
    if (rdia.comboBox_S_4->currentText() != "") {
      AddMalzgabe(rdia.comboBox_S_4->currentText(),
                  rdia.doubleSpinBox_Prozent_S_4->value(), double(0),
                  double(0));
    }
    if (rdia.comboBox_S_5->currentText() != "") {
      AddMalzgabe(rdia.comboBox_S_5->currentText(),
                  rdia.doubleSpinBox_Prozent_S_5->value(), double(0),
                  double(0));
    }
    if (rdia.comboBox_S_6->currentText() != "") {
      AddMalzgabe(rdia.comboBox_S_6->currentText(),
                  rdia.doubleSpinBox_Prozent_S_6->value(), double(0),
                  double(0));
    }
    BerAlles();
  }
...
void MainWindowImpl::on_tableWidget_Schnellgaerverlauf_itemChanged(
    QTableWidgetItem *item) {
  if (Gestartet && !AmLaden) {
    ...
    double sw, alc;
    zeile = item->row();
    sw = tableWidget_Schnellgaerverlauf->item(zeile, 1)
             ->data(Qt::DisplayRole)
             .toDouble();
    // Alkoholgehalt berechnen
    alc = Berechnungen.BerAlkohoVol(spinBox_SWSollGesammt->value(), sw);
    alc = double(qRound(alc * 10)) / 10;
    tableWidget_Schnellgaerverlauf->item(zeile, 2)->setData(Qt::DisplayRole,
                                                            alc);
    tableWidget_Schnellgaerverlauf->setSortingEnabled(true);
    tableWidget_Schnellgaerverlauf->setSortingEnabled(false);
    setAenderung(true);
...
void MainWindowImpl::on_tableWidget_Hauptgaerverlauf_itemChanged(
    QTableWidgetItem *item) {
  if (Gestartet && !AmLaden) {
    // Alkoholgehalt dieser Zeile neu berechnen
    int zeile;
    double sw, alc;
    zeile = item->row();
    sw = tableWidget_Hauptgaerverlauf->item(zeile, 1)
             ->data(Qt::DisplayRole)
             .toDouble();
    // Alkoholgehalt berechnen
    alc = Berechnungen.BerAlkohoVol(spinBox_SWSollGesammt->value(), sw);
...
void MainWindowImpl::on_tableWidget_Nachgaerverlauf_itemChanged(
  ...
  if (Gestartet && !AmLaden) {
    ...
    double druck, temp, co2;
    zeile = item->row();
    druck = tableWidget_Nachgaerverlauf->item(zeile, 1)
                ->data(Qt::DisplayRole)
                .toDouble();
    temp = tableWidget_Nachgaerverlauf->item(zeile, 2)
               ->data(Qt::DisplayRole)
               .toDouble();
    co2 = Berechnungen.BerCO2Gehalt(druck, temp);
    co2 = double(qRound(co2 * 100)) / 100;
...
void MainWindowImpl::DBErgebnisseNeuBerechnen() {
  // Von Allen Suden die EffektiveAusbeute neu Berechnen da bisher die
  // abgezwackte
  ...
  if (!query.exec(sql)) {
    ...
    int id;
    int FeldNr;
    // Alle Sude die gebraut wurden durchgehen und Effektive Ausbeute neu
    // berechnen
    while (query.next()) {
      ...
      if (NeuBerechnen == 1) {
        ...
        sql2 = "UPDATE 'Sud' SET 'erg_EffektiveAusbeute'=" +
               QString::number(effausbeute) +
      ...
      if (NeuBerechnen == 1) {
        sudhausausbeute =
            Berechnungen.GetSudhausausbeute(sw, menge, schuettung);
        // Sudhausausbeute in DB Schreiben
        sql2 = "UPDATE 'Sud' SET 'erg_Sudhausausbeute'=" +
               QString::number(sudhausausbeute) +
               " WHERE ID=" + QString::number(id);
        if (!query2.exec(sql2)) {
          ErrorMessage *errorMessage = new ErrorMessage();
          errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
      ...
      if (NeuBerechnen == 2) {
        ...
        // Grünschlauchzeitpunkt
        Berechnungen.GetGruenschlauchzeitpunkt(co2, TempJungbier,
                                               SWSchnellgaerprobe);
        ...
        // Erforderliche Speisemenge
        double SpeisemengeGesammt =
            Berechnungen.GetSpeiseGesammt(SWAnstellen, SWJungbier,
                                          JungbiermengeAbfuellen,
                                          AbgefuellteSpeisemenge, false) /
            1000;

        // Abgefuellte Menge
        double AbgefuellteMenge = JungbiermengeAbfuellen + SpeisemengeGesammt;
        sql2 = "UPDATE 'Sud' SET 'erg_AbgefuellteBiermenge'=" +
               QString::number(AbgefuellteMenge) +
               " WHERE ID=" + QString::number(id);
        if (!query2.exec(sql2)) {
          ErrorMessage *errorMessage = new ErrorMessage();
          errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
...
void MainWindowImpl::BerAusruestung() {
  ...
  Grundflaeche =
      pow(spinBox_MaischebottichDurchmesser->value() / 2, 2) * M_PI / 10;
  spinBox_MaischenVolumen->setValue(Grundflaeche *
                                    spinBox_MaischebottichHoehe->value() / 100);
  spinBox_MaischenMaxNutzvolumen->setValue(
      Grundflaeche * spinBox_MaischebottichMaxFuellhoehe->value() / 100);

  // Volumen Sudpfanne berechnen
  Grundflaeche = pow(spinBox_SudpfanneDurchmesser->value() / 2, 2) * M_PI / 10;
  spinBox_SudpfanneVolumen->setValue(Grundflaeche *
                                     spinBox_SudpfanneHoehe->value() / 100);
  spinBox_SudpfanneMaxNutzvolumen->setValue(
      Grundflaeche * spinBox_SudpfanneMaxFuellhoehe->value() / 100);
}
...
void MainWindowImpl::BerEmpfehlungHefeEinheiten() {
  ...
  if (s != "") {
    ...
    for (int i = 0; i < AnzahlHefeEintraege; i++) {
      // wenn Eintrag übereinstimmt
      if (tableWidget_Hefe->item(i, TableHefeColName)->text() == s) {
        QSpinBox *spinBox = (QSpinBox *)tableWidget_Hefe->cellWidget(
            i, TableHefeColWuerzemenge);
...
void MainWindowImpl::setHefeAuswahlListeFarbe() {
  ...
  for (int i = 0; i < comboBox_AuswahlHefe->count(); i++) {
    ...
    if (menge == 0) {
      if (StyleDunkel)
        comboBox_AuswahlHefe->setItemData(
            i, QColor::fromRgb(FARBE_COMBO_ROHSTOFF_EMPTY_DUNKEL),
            Qt::TextColorRole);
      else
        comboBox_AuswahlHefe->setItemData(
            i, QColor::fromRgb(FARBE_COMBO_ROHSTOFF_EMPTY_HELL),
            Qt::TextColorRole);
    } else if (menge < spinBox_AnzahlHefeEinheiten->value()) {
      if (StyleDunkel)
        comboBox_AuswahlHefe->setItemData(
            i, QColor::fromRgb(FARBE_COMBO_ROHSTOFF_LOW_DUNKEL),
            Qt::TextColorRole);
      else
        comboBox_AuswahlHefe->setItemData(
            i, QColor::fromRgb(FARBE_COMBO_ROHSTOFF_LOW_HELL),
            Qt::TextColorRole);
    } else {
      if (StyleDunkel)
        comboBox_AuswahlHefe->setItemData(i, QColor(Qt::white),
                                          Qt::TextColorRole);
      else
        comboBox_AuswahlHefe->setItemData(i, QColor(40, 40, 40),
                                          Qt::TextColorRole);
    }
  }
}
...
void MainWindowImpl::slot_ResetBierWurdeGebraut() {
  QSqlQuery query;
  QString sql = "UPDATE Sud SET BierWurdeGebraut=0 WHERE ID=" +
                QString::number(AktuelleSudID) + ";";
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
...
void MainWindowImpl::slot_ResetAbgefuellt() {
  QSqlQuery query;
  QString sql = "UPDATE Sud SET BierWurdeAbgefuellt=0 WHERE ID=" +
                QString::number(AktuelleSudID) + ";";
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
...
void MainWindowImpl::slot_ResetBierVerbraucht() {
  QSqlQuery query;
  QString sql = "UPDATE Sud SET BierWurdeVerbraucht=0 WHERE ID=" +
                QString::number(AktuelleSudID) + ";";
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
...
void MainWindowImpl::BerVolumenMaischen() {
  ...
  // Näherungsweise rechnet man mit
  ...
  doubleSpinBox_VolumenMaische->setValue(
      wasser + (doubleSpinBox_S_Gesammt->value() * 0.7));
}

void MainWindowImpl::BerPfanneVoll() {
  doubleSpinBox_VolumenPfannevoll->setValue(Berechnungen.GetPfanneVoll(
      getVerdampfungsziffer(), spinBox_Gesammtkochdauer->value(),
      spinBox_Menge->value() / highGravityFaktor));
}
...
void MainWindowImpl::on_pushButton_SudExport_clicked() {
  ...
  // Sudname anhängen
  p += "/" + Sudname + ".xsud";
  s = QFileDialog::getSaveFileName(this, trUtf8("Export Sud"), p,
                                   trUtf8("KBH Sud Export Dateien (*.xsud)") +
                                       ";;" + trUtf8("BeerXML (*.xml)"),
                                   0);
  if (!s.isEmpty()) {
...
void MainWindowImpl::on_pushButton_SudImport_clicked() {
  ...
  if (!s.isEmpty()) {
    ...
    // Fehler beim Parsen der Datei
      ...
      ErrorMessage *errorMessage = new ErrorMessage();
      errorMessage->showMessage(
          ERR_XML_PARSEN, TYPE_WARNUNG, CANCEL_NO,
          trUtf8("Dateiname: ") + s + "\n" + trUtf8("Message: ") +
              Export.errMsg + "\n" + trUtf8("Zeile: ") +
    ...
    // Fehler beim Auslesen der Suddaten
      ...
      // Sudauswahl aktuallisieren
      FuelleSudauswahl();
      // Rohstofftabellen neu einlesen für den Fall das neue Rohstoffe
      // hinzugefügt wurden
      Gestartet = false;
      LeseRohstoffeDB();
      AenderungRohstofftabelle = true;
...
void MainWindowImpl::on_SpinBox_wwSaeurekapazitaet_mmol_valueChanged(double) {
  if (SpinBox_wwSaeurekapazitaet_mmol->hasFocus()) {
    SpinBox_wwCarbonathaerte_dh->setValue(
        SpinBox_wwSaeurekapazitaet_mmol->value() * 2.8);
    BerRestalkalitaet();
  }
}
...
void MainWindowImpl::on_SpinBox_wwCarbonathaerte_dh_valueChanged(double) {
  if (SpinBox_wwCarbonathaerte_dh->hasFocus()) {
    SpinBox_wwSaeurekapazitaet_mmol->setValue(
        SpinBox_wwCarbonathaerte_dh->value() / 2.8);
    BerRestalkalitaet();
  }
}
...
void MainWindowImpl::BerWasserwerte() {
  ...
  SpinBox_waSauermalz_g->setValue(doubleSpinBox_S_Gesammt->value() / 100 *
                                  SpinBox_waSauermalz_pr->value() * 1000);
  // Nachguss
...
void MainWindowImpl::BerRestalkalitaet() {
  ...
  d = SpinBox_wwCarbonathaerte_dh->value() -
      ((SpinBox_wwCalciumhaerte_dh->value() +
        0.5 * SpinBox_wwMagnesiumhaerte_dh->value()) /
       3.5);
...
void MainWindowImpl::on_pushButton_WeitereZutatenDel_clicked() {
  ...
  QString del_name =
      tableWidget_WeitereZutaten
          ->item(tableWidget_WeitereZutaten->currentRow(), TableWZutatColName)
          ->text();
  ...
  if (ok) {
    tableWidget_WeitereZutaten->removeRow(
        tableWidget_WeitereZutaten->currentRow());
    setAenderung(true);
    AenderungRohstofftabelle = true;
  }
}
...
void MainWindowImpl::on_pushButton_EWZ_Hinzufuegen_clicked() {
  ...
  if (BierWurdeGebraut) {
    ewz->setZeitpunkt(EWZ_Zeitpunkt_Gaerung);
  }
...
void MainWindowImpl::slot_rastAenderung(int) { setAenderung(true); }
...
int MainWindowImpl::slot_getEwzTyp(QString zutat) {
  for (int i = 0; i < tableWidget_WeitereZutaten->rowCount(); i++) {
    if (tableWidget_WeitereZutaten->item(i, TableWZutatColName)->text() ==
        zutat) {
      QComboBox *comboTyp = (QComboBox *)tableWidget_WeitereZutaten->cellWidget(
          i, TableWZutatColTyp);
...
int MainWindowImpl::slot_getEwzAusbeute(QString zutat) {
  for (int i = 0; i < tableWidget_WeitereZutaten->rowCount(); i++) {
    if (tableWidget_WeitereZutaten->item(i, TableWZutatColName)->text() ==
        zutat) {
      QDoubleSpinBox *spinBoxAusbeute =
          (QDoubleSpinBox *)tableWidget_WeitereZutaten->cellWidget(
              i, TableWZutatColAusbeute);
...
void MainWindowImpl::SchreibeErweiterteZutatenDB() {
  ...
  // und dann neu schreiben
  QSqlQuery query;
  QString sql = "DELETE FROM WeitereZutatenGaben WHERE SudID =" +
                QString::number(AktuelleSudID);
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
  ...
  for (int i = 0; i < list_EwZutat.count(); i++) {
    sql = "INSERT INTO WeitereZutatenGaben(SudID, Name, Menge, Einheit, Typ, "
          "Zeitpunkt,";
    sql += "Bemerkung, erg_Menge, Ausbeute, Zeitpunkt_von, Zeitpunkt_bis, "
           "Entnahmeindex, Zugabestatus, Zugabedauer, Farbe) VALUES(" +
           QString::number(AktuelleSudID) + "," + "'" +
...
double MainWindowImpl::slot_EwzGetMenge(QString name) {
  ...
  double verwendet = 0;
  for (int o = 0; o < list_EwZutat.count(); o++) {
    if ((list_EwZutat[o]->getName() == name) &&
        (list_EwZutat[o]->getTyp() != EWZ_Typ_Hopfen)) {
...
void MainWindowImpl::BerWeitereZutaten() {
  ...
  for (int i = 0; i < list_EwZutat.count(); i++) {
    list_EwZutat[i]->setErg_Menge(list_EwZutat[i]->getMenge() *
                                  spinBox_Menge->value());
  }
  sw_ewz = 0;
  ...
  for (int i = 0; i < list_EwZutat.count(); i++) {
    // wenn die erweiterte Zutat eine Ausbeute über 0 hat wird sie
    // berücksichtigt
    if (list_EwZutat[i]->getAusbeute() > 0) {
      // wenn Typ Hopfen ist ignorieren
      if (list_EwZutat[i]->getTyp() != 100) {
        gs_ewz +=
            list_EwZutat[i]->getMenge() * list_EwZutat[i]->getAusbeute() / 100;
        // wenn die zutat schon beim kochen dabei ist
        if (list_EwZutat[i]->getZeitpunkt() > 0) {
          gs_ewz_kochen += list_EwZutat[i]->getMenge() *
                           list_EwZutat[i]->getAusbeute() / 100;
        }
      }
    }
  }
  // Berechnen wieviel der soll-Stammwürze durch die erweiterten Zutaten
  // erziehlt wird.
  if (BierWurdeGebraut) {
...
void MainWindowImpl::slot_pushButton_RastNachOben(int id) {
  ...
  if (id != list_Rasten[0]->getID()) {
    ...
    for (int o = 0; o < verticalLayout_Rasten->count(); o++) {
      verticalLayout_Rasten->removeWidget(
          verticalLayout_Rasten->itemAt(0)->widget());
    }
...
void MainWindowImpl::on_spinBox_WuerzemengeVorHopfenseihen_valueChanged(
    double arg1) {
  if (Gestartet) {
...
void MainWindowImpl::slot_BewertungWoche_clicked(int id) {
  ...
  if (index > -1) {
    ...
    dateEdit_Bewertungsdatum->setDate(
        list_Bewertung[index]->getBewertungdatum());
    // woche
    label_bewWoche->setText(trUtf8("Woche ") +
                            QString::number(list_Bewertung[index]->getWoche()));
    // Sterne
    ...
    textEdit_haupttrunk->setText(
        list_Bewertung[index]->getHaupttrunkBemerkung());
    textEdit_nachtrunk->setText(list_Bewertung[index]->getNachtrunkBemerkung());
    textEdit_gesamteindruck->setText(
        list_Bewertung[index]->getGesamteindruckBemerkung());
...
void MainWindowImpl::on_dateEdit_Bewertungsdatum_dateChanged(
    const QDate &date) {
  int index = getBewertungsIndex();
...
void MainWindowImpl::on_textEdit_bewBemerkung_textChanged() {
  list_Bewertung[getBewertungsIndex()]->setBemerkung(
      textEdit_bewBemerkung->document()->toPlainText());
  setAenderung(true);
}

void MainWindowImpl::on_radioButton_farbe_0_clicked() { setBewertungFarbe(); }

void MainWindowImpl::on_radioButton_farbe_1_clicked() { setBewertungFarbe(); }

void MainWindowImpl::on_radioButton_farbe_2_clicked() { setBewertungFarbe(); }

void MainWindowImpl::on_radioButton_farbe_3_clicked() { setBewertungFarbe(); }

void MainWindowImpl::on_radioButton_farbe_4_clicked() { setBewertungFarbe(); }

void MainWindowImpl::on_radioButton_farbe_5_clicked() { setBewertungFarbe(); }

void MainWindowImpl::on_radioButton_farbe_6_clicked() { setBewertungFarbe(); }

void MainWindowImpl::on_radioButton_farbe_7_clicked() { setBewertungFarbe(); }

void MainWindowImpl::on_radioButton_farbe_8_clicked() { setBewertungFarbe(); }

void MainWindowImpl::on_radioButton_farbe_9_clicked() { setBewertungFarbe(); }

void MainWindowImpl::on_radioButton_farbe_10_clicked() { setBewertungFarbe(); }

void MainWindowImpl::on_radioButton_schaum_0_clicked() { setBewertungSchaum(); }

void MainWindowImpl::on_radioButton_schaum_1_clicked() { setBewertungSchaum(); }

void MainWindowImpl::on_radioButton_schaum_2_clicked() { setBewertungSchaum(); }

void MainWindowImpl::on_radioButton_schaum_3_clicked() { setBewertungSchaum(); }

void MainWindowImpl::on_radioButton_schaum_4_clicked() { setBewertungSchaum(); }

void MainWindowImpl::on_radioButton_schaum_5_clicked() { setBewertungSchaum(); }

void MainWindowImpl::on_radioButton_schaum_6_clicked() { setBewertungSchaum(); }

void MainWindowImpl::on_radioButton_schaum_7_clicked() { setBewertungSchaum(); }

void MainWindowImpl::on_radioButton_schaum_8_clicked() { setBewertungSchaum(); }

void MainWindowImpl::on_radioButton_schaum_9_clicked() { setBewertungSchaum(); }

void MainWindowImpl::on_radioButton_schaum_10_clicked() {
  setBewertungSchaum();
}

void MainWindowImpl::on_checkBox_geruch_0_clicked() { setBewertungGeruch(); }

void MainWindowImpl::on_checkBox_geruch_1_clicked() { setBewertungGeruch(); }

void MainWindowImpl::on_checkBox_geruch_2_clicked() { setBewertungGeruch(); }

void MainWindowImpl::on_checkBox_geruch_3_clicked() { setBewertungGeruch(); }

void MainWindowImpl::on_checkBox_geruch_4_clicked() { setBewertungGeruch(); }

void MainWindowImpl::on_checkBox_geruch_5_clicked() { setBewertungGeruch(); }

void MainWindowImpl::on_checkBox_geruch_6_clicked() { setBewertungGeruch(); }

void MainWindowImpl::on_checkBox_geruch_7_clicked() { setBewertungGeruch(); }

void MainWindowImpl::on_checkBox_geruch_8_clicked() { setBewertungGeruch(); }

void MainWindowImpl::on_checkBox_geruch_9_clicked() { setBewertungGeruch(); }

void MainWindowImpl::on_checkBox_geruch_10_clicked() { setBewertungGeruch(); }

void MainWindowImpl::on_checkBox_geruch_11_clicked() { setBewertungGeruch(); }
...
void MainWindowImpl::on_textEdit_farbe_textChanged() {
  list_Bewertung[getBewertungsIndex()]->setFarbeBemerkung(
      textEdit_farbe->document()->toPlainText());
  setAenderung(true);
}

void MainWindowImpl::on_textEdit_schaum_textChanged() {
  list_Bewertung[getBewertungsIndex()]->setSchaumBemerkung(
      textEdit_schaum->document()->toPlainText());
  setAenderung(true);
}

void MainWindowImpl::on_textEdit_geruch_textChanged() {
  list_Bewertung[getBewertungsIndex()]->setGeruchBemerkung(
      textEdit_geruch->document()->toPlainText());
  setAenderung(true);
}

void MainWindowImpl::on_textEdit_geschmack_textChanged() {
  list_Bewertung[getBewertungsIndex()]->setGeschmackBemerkung(
      textEdit_geschmack->document()->toPlainText());
  setAenderung(true);
}

void MainWindowImpl::on_textEdit_antrunk_textChanged() {
  list_Bewertung[getBewertungsIndex()]->setAntrunkBemerkung(
      textEdit_antrunk->document()->toPlainText());
  setAenderung(true);
}

void MainWindowImpl::on_textEdit_haupttrunk_textChanged() {
  list_Bewertung[getBewertungsIndex()]->setHaupttrunkBemerkung(
      textEdit_haupttrunk->document()->toPlainText());
  setAenderung(true);
}

void MainWindowImpl::on_textEdit_nachtrunk_textChanged() {
  list_Bewertung[getBewertungsIndex()]->setNachtrunkBemerkung(
      textEdit_nachtrunk->document()->toPlainText());
  setAenderung(true);
}

void MainWindowImpl::on_textEdit_gesamteindruck_textChanged() {
  list_Bewertung[getBewertungsIndex()]->setGesamteindruckBemerkung(
      textEdit_gesamteindruck->document()->toPlainText());
  setAenderung(true);
}
...
void MainWindowImpl::on_tableWidget_Malz_cellChanged(int row, int column) {
  if (Gestartet) {
    // Wenn sich der Malz eintrag geändert hat dann Sude nach verwendung
    // durchsuchen und abändern
    ...
    while (doppelt) {
      ...
      for (int i = 0; i < tableWidget_Malz->rowCount(); i++) {
        if (i != row) {
          if (tableWidget_Malz->item(i, TableMalzColName)->text() ==
              newItem->text()) {
            doppelt = true;
    ...
    if ((Malz_Bezeichnung_Merker != "") && (column == 0)) {
      if (Malz_Bezeichnung_Merker != newItem->text()) {
        ...
        if (!query.exec(sql)) {
          ...
          while (query.next()) {
            QSqlQuery query2;
            // Bei allen gefundenen Suden eventuell vorhandene
            // Schüttungseinträge austauschen
            int FeldNr = query.record().indexOf("ID");
            QString str = Malz_Bezeichnung_Merker;
            QString sql2 = "UPDATE Malzschuettung SET Name='" +
                           newItem->text().replace("'", "''") +
                           "' WHERE Name='" + str.replace("'", "''") +
                           "' AND SudID=" + query.value(FeldNr).toString();
            if (!query2.exec(sql2)) {
              // Fehlermeldung Datenbankabfrage
              ErrorMessage *errorMessage = new ErrorMessage();
              errorMessage->showMessage(
        ...
        // Da eventuell Datenbankeinträge der Sude geändert wurden müssen die
        // Rohstoffe jetzt auch gespeichert werden
        // da ansonsten (wenn beim beenden nicht geseichert wird) die
        // Rohstoffeinträge nicht mehr zu den Suden passen
...
void MainWindowImpl::on_tableWidget_Hopfen_cellChanged(int row, int column) {
  if (Gestartet) {
    // Wenn sich der Hopfen eintrag geändert hat dann Sude nach verwendung
    // durchsuchen und abändern
    QTableWidgetItem *newItem =
        tableWidget_Hopfen->item(row, TableHopfenColName);
...
void MainWindowImpl::on_tableWidget_Hefe_cellChanged(int row, int column) {
  if (Gestartet) {
    // Wenn sich der Hefe eintrag geändert hat dann Sude nach verwendung
    // durchsuchen und abändern
    ...
    while (doppelt) {
      ...
      for (int i = 0; i < tableWidget_Hefe->rowCount(); i++) {
        if (i != row) {
          if (tableWidget_Hefe->item(i, TableHefeColName)->text() ==
              newItem->text()) {
            doppelt = true;
    ...
    if ((Hefe_Bezeichnung_Merker != "") && (column == 0)) {
      if (Hefe_Bezeichnung_Merker != newItem->text()) {
        ...
        QString sql = "UPDATE Sud SET AuswahlHefe='" +
                      newItem->text().replace("'", "''") +
                      "' WHERE BierWurdeGebraut == 0 AND AuswahlHefe=='" +
                      str.replace("'", "''") + "'";
        if (!query.exec(sql)) {
          // Fehlermeldung Datenbankabfrage
          ErrorMessage *errorMessage = new ErrorMessage();
...
void MainWindowImpl::on_tableWidget_WeitereZutaten_cellChanged(int row,
                                                               int column) {
  if (Gestartet) {
    // Wenn sich der Eintrag geändert hat dann Sude nach verwendung durchsuchen
    // und abändern
    QTableWidgetItem *newItem =
        tableWidget_WeitereZutaten->item(row, TableWZutatColName);
      ...
      for (int i = 0; i < tableWidget_WeitereZutaten->rowCount(); i++) {
        if (i != row) {
          if (tableWidget_WeitereZutaten->item(i, TableWZutatColName)->text() ==
              newItem->text()) {
            doppelt = true;
            // qDebug() << "Eintrag in den Weiteren Zutaten ist doppelt: " <<
            // newItem->text();
            newItem->setText(newItem->text() + "_");
    ...
    if ((WZutaten_Bezeichnung_Merker != "") && (column == 0)) {
      if (WZutaten_Bezeichnung_Merker != newItem->text()) {
        ...
        // Da eventuell Datenbankeinträge der Sude geändert wurden müssen die
        // Rohstoffe jetzt auch gespeichert werden
        // da ansonsten (wenn beim beenden nicht gespeichert wird) die
        // Rohstoffeinträge nicht mehr zu den Suden passen
...
void MainWindowImpl::on_tableWidget_Malz_currentCellChanged(int currentRow,
                                                            int currentColumn,
                                                            int, int) {
  // wenn eine Zelle von der Beschreibung angewählt wurde Text merken für
  // späteren vergleich ob sich Beschreibungstext geändert hat
  // und Sude angepasst werden müssen
  if (currentColumn == 0) {
    QTableWidgetItem *newItem =
        tableWidget_Malz->item(currentRow, currentColumn);
    if (newItem)
      Malz_Bezeichnung_Merker = newItem->text();
  }
}

void MainWindowImpl::on_tableWidget_Hopfen_currentCellChanged(int currentRow,
                                                              int currentColumn,
                                                              int, int) {
  // wenn eine Zelle von der Beschreibung angewählt wurde Text merken für
  // späteren vergleich ob sich Beschreibungstext geändert hat
  ...
  if (currentColumn == 0) {
    QTableWidgetItem *newItem =
        tableWidget_Hopfen->item(currentRow, currentColumn);
    if (newItem)
      Hopfen_Bezeichnung_Merker = newItem->text();
  }
}

void MainWindowImpl::on_tableWidget_Hefe_currentCellChanged(int currentRow,
                                                            int currentColumn,
                                                            int, int) {
  // wenn eine Zelle von der Beschreibung angewählt wurde Text merken für
  // späteren vergleich ob sich Beschreibungstext geändert hat
  ...
  if (currentColumn == 0) {
    QTableWidgetItem *newItem =
        tableWidget_Hefe->item(currentRow, currentColumn);
    if (newItem)
      Hefe_Bezeichnung_Merker = newItem->text();
  }
}

void MainWindowImpl::on_tableWidget_WeitereZutaten_currentCellChanged(
  ...
  if (currentColumn == 0) {
    QTableWidgetItem *newItem =
        tableWidget_WeitereZutaten->item(currentRow, currentColumn);
    if (newItem)
      WZutaten_Bezeichnung_Merker = newItem->text();
  }
}
...
void MainWindowImpl::on_pushButton_EingabeHVerdampfungsziffer_clicked() {
  ...
  bver.setMenge2(Berechnungen.BerVolumenWasser(
      20, 99, spinBox_WuerzemengeKochende->value()));
...
void MainWindowImpl::on_listWidget_Brauanlagen_itemSelectionChanged() {
  if ((listWidget_Brauanlagen->selectedItems().count() == 1) &&
      (listWidget_Brauanlagen->count() > 1)) {
  ...
  if (listWidget_Brauanlagen->selectedItems().count() == 1) {
    ...
    // Maischebottich Durchmesser
    spinBox_MaischebottichDurchmesser->setValue(
        item->getMaischebottich_Durchmesser());
    // Maischebottich Maximal nutzbare Füllhöhe
    spinBox_MaischebottichMaxFuellhoehe->setValue(
        item->getMaischebottich_MaxFuellhoehe());
    // Sudpfanne Höhe
    ...
    // Sudpfanne Maximal nutzbare Füllhöhe
    spinBox_SudpfanneMaxFuellhoehe->setValue(
        item->getSudpfanne_MaxFuellhoehe());
    BerEffektiveAusbeuteMittel();
    Gestartet = merker;
    BerAusruestung();
...
void MainWindowImpl::on_spinBox_MaischebottichDurchmesser_valueChanged(
    double arg1) {
  if (Gestartet) {
    setAenderung(true);
    AenderungAusruestung = true;
...
void MainWindowImpl::on_spinBox_MaischebottichMaxFuellhoehe_valueChanged(
    double arg1) {
  if (Gestartet) {
    setAenderung(true);
    AenderungAusruestung = true;
...
void MainWindowImpl::on_spinBox_SudpfanneMaxFuellhoehe_valueChanged(
    double arg1) {
  if (Gestartet) {
    setAenderung(true);
    AenderungAusruestung = true;
...
void MainWindowImpl::on_hSlider_dAusbeuteSude_valueChanged(int) {
  ...
  sql = "SELECT erg_EffektiveAusbeute FROM Sud WHERE BierWurdeGebraut=1 AND "
        "AusbeuteIgnorieren=0 AND AuswahlBrauanlage=" +
        QString::number(AnlagenID) + " ORDER BY Braudatum DESC LIMIT " +
...
void MainWindowImpl::on_pushButton_merken_clicked() {
  ...
  if (row >= 0) {
    ...
    QString sql;
    sql =
        "UPDATE 'Sud' SET 'MerklistenID'=1 WHERE ID=" + QString::number(SudID);
    if (!query.exec(sql)) {
      // Fehlermeldung Datenbankabfrage
      ErrorMessage *errorMessage = new ErrorMessage();
      errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
...
void MainWindowImpl::on_pushButton_vergessen_clicked() {
  ...
  if (row >= 0) {
    ...
    QString sql;
    sql =
        "UPDATE 'Sud' SET 'MerklistenID'=0 WHERE ID=" + QString::number(SudID);
    if (!query.exec(sql)) {
      // Fehlermeldung Datenbankabfrage
      ErrorMessage *errorMessage = new ErrorMessage();
      errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
...
void MainWindowImpl::on_pushButton_MalzNeu_clicked() {
  ...
  QString s = Malz_Bezeichnung_Merker;
  Malz_Bezeichnung_Merker = "";
  MalzNeueZeile(trUtf8("Neuer Eintrag"), 0.0, 100, 0.0, 0.0, "", "",
                QDate::currentDate(), QDate::currentDate().addMonths(1), "");
  setAenderung(true);
  AenderungRohstofftabelle = true;
  Malz_Bezeichnung_Merker = s;
...
void MainWindowImpl::on_pushButton_HopfenNeu_clicked() {
  ...
  QString s = Hopfen_Bezeichnung_Merker;
  Hopfen_Bezeichnung_Merker = "";
  HopfenNeueZeile(trUtf8("Neuer Eintrag"), 0.0, 0.0, 0.0, true, "", 0, "",
                  QDate::currentDate(), QDate::currentDate().addMonths(1), "");
  setAenderung(true);
  AenderungRohstofftabelle = true;
  Hopfen_Bezeichnung_Merker = s;
...
void MainWindowImpl::on_pushButton_HefeNeu_clicked() {
  ...
  QString s = Hefe_Bezeichnung_Merker;
  Hefe_Bezeichnung_Merker = "";
  HefeNeueZeile(trUtf8("Neuer Eintrag"), 0.0, 0.0, 0.0, "", "", 0, 0, "", "", 0,
                "", QDate::currentDate(), QDate::currentDate().addMonths(1),
                "");
  setAenderung(true);
  AenderungRohstofftabelle = true;
...
void MainWindowImpl::on_pushButton_WeitereZutatenNeu_clicked() {
  ...
  QString s = WZutaten_Bezeichnung_Merker;
  WZutaten_Bezeichnung_Merker = "";
  WeitereZutatNeueZeile(trUtf8("Neuer Eintrag"), 0.0, 0, 4, 0.0, 0.0, 0.0, "",
                        QDate::currentDate(), QDate::currentDate().addMonths(1),
                        "");
  setAenderung(true);
  AenderungRohstofftabelle = true;
...
void MainWindowImpl::on_listWidget_Brauanlagen_currentItemChanged(
    QListWidgetItem *, QListWidgetItem *previous) {
  if (Gestartet) {
  ...
  if (column == TableMalzColLink) {
    if (QApplication::keyboardModifiers() & Qt::ControlModifier)
      QDesktopServices::openUrl(
          QUrl(tableWidget_Malz->item(row, column)->text()));
  }
}
...
void MainWindowImpl::on_tableWidget_Hopfen_cellClicked(int row, int column) {
  if (column == TableHopfenColLink) {
    if (QApplication::keyboardModifiers() & Qt::ControlModifier)
      QDesktopServices::openUrl(
          QUrl(tableWidget_Hopfen->item(row, column)->text()));
  }
}
...
void MainWindowImpl::on_tableWidget_Hefe_cellClicked(int row, int column) {
  if (column == TableHefeColLink) {
    if (QApplication::keyboardModifiers() & Qt::ControlModifier)
      QDesktopServices::openUrl(
          QUrl(tableWidget_Hefe->item(row, column)->text()));
  }
}

void MainWindowImpl::on_tableWidget_WeitereZutaten_cellClicked(int row,
                                                               int column) {
  if (column == TableWZutatColLink) {
    if (QApplication::keyboardModifiers() & Qt::ControlModifier)
      QDesktopServices::openUrl(
          QUrl(tableWidget_WeitereZutaten->item(row, column)->text()));
  }
}
...
void MainWindowImpl::on_pushButton_SW_Info_clicked() {
  DialogInfo::Info(this, trUtf8("Stammwürze"),
                   trUtf8("<b>Typische Stammwürze in °P:</b></br><table>\
...
void MainWindowImpl::AddAnhang(QString pfad) {
  ...
  if (anhang->getPfad() != "") {
    ...
    connect(anhang, SIGNAL(sig_vorClose(int)), this,
            SLOT(slot_anhangClose(int)));
    connect(anhang, SIGNAL(sig_Aenderung()), this,
            SLOT(slot_anhangAenderung()));
  }
}
...
void MainWindowImpl::slot_anhangAenderung() { setAenderung(true); }

void MainWindowImpl::LeseAnhangDB() {
  // Anhang Abfragen
  QSqlQuery query_anhang;
  QString sql = "SELECT * FROM Anhang WHERE SudID=" +
                QString::number(AktuelleSudID) + ";";
...
void MainWindowImpl::SchreibeAnhangDB() {
  QSqlQuery query;
  QString sql =
      "DELETE FROM Anhang WHERE SudID =" + QString::number(AktuelleSudID);
  if (!query.exec(sql)) {
    // Fehlermeldung Datenbankabfrage
    ErrorMessage *errorMessage = new ErrorMessage();
    errorMessage->showMessage(ERR_SQL_DB_ABFRAGE, TYPE_WARNUNG, CANCEL_NO,
...
void MainWindowImpl::on_pushButton_CalcEinmaischeTemp_clicked() {
  DialogEinmaischeTemp *dlg = new DialogEinmaischeTemp(
      doubleSpinBox_S_Gesammt->value(), 18.0, doubleSpinBox_WHauptguss->value(),
      list_Rasten.count() > 0 ? list_Rasten[0]->getRastTemp() : 57.0, this);
...
void MainWindowImpl::on_pushButton_SudTeilen_clicked() {
  if (Aenderung ? AbfrageSpeichern() : true) {
    DialogSudTeilen *dlg = new DialogSudTeilen(
        lineEdit_Sudname->text(), spinBox_WuerzemengeAnstellen->value(), this);
    if (dlg->exec() == QDialog::Accepted) {
      int id = Database::SudKopieren(QString::number(AktuelleSudID),
                                     dlg->nameTeil2(), true);
      if (id > 0) {
        double factor = 0.0;
        ...
        spinBox_WuerzemengeKochende->setValue(
            spinBox_WuerzemengeKochende->value() * factor);
        spinBox_Speisemenge->setValue(spinBox_Speisemenge->value() * factor);
        spinBox_WuerzemengeAnstellen->setValue(
            spinBox_WuerzemengeAnstellen->value() * factor);
        spinBox_JungbiermengeAbfuellen->setValue(
            spinBox_JungbiermengeAbfuellen->value() * factor);
        spinBox_AnzahlHefeEinheiten->setValue(
            qRound(spinBox_AnzahlHefeEinheiten->value() * factor));
        Gestartet = true;
        if (wurdeGebraut) {
